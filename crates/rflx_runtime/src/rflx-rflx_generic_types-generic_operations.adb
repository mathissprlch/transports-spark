...
is
   ...
   with
     Pre  =>
       ...
       and then Long_Integer (((Natural (Off) + Value_Size) - 1) / Byte'Size)
                < Long_Integer (Last - First + 1),
     Post =>
       RME_Index = Index (Last - Long_Integer (Off) / Byte'Size)
       and then LME_Index
                = Index
                    (Last
                     - ((Long_Integer (Off) + Long_Integer (Value_Size)) - 1)
                       / Byte'Size)
       and then RME_Size = Byte'Size - Natural (Off)
       and then LME_Size
                = ((Natural (Off) + Value_Size + Byte'Size) - 1)
                  mod Byte'Size
                  + 1
   is
   begin
      ...
      LME_Size :=
        ((Natural (Off) + Value_Size + Byte'Size) - 1) mod Byte'Size + 1;
   ...
   with
     ...
     Post => (if Value_Size < U64'Size then U64_Extract'Result < 2**Value_Size)
   is
   ...
   begin
      Get_Index_Offset
        (Long_Integer (Data'First),
         Long_Integer (Data'Last),
         Off,
         Value_Size,
         RME_Index,
         LME_Index,
      ...
      for I in LME_Index + 1 .. RME_Index - 1 loop
         ...
         pragma
           Loop_Invariant
             (Fits_Into
                (Result,
                 Natural (I - LME_Index + 1) * Byte'Size - LME_Offset));
      end loop;
      pragma Assert (RME_Size in 1 .. U64'Size);
      pragma
        Assert
          (if LME_Index + 1 <= RME_Index - 1
             then
               Fits_Into
                 (Result,
                  Natural (RME_Index - LME_Index) * Byte'Size - LME_Offset));
      pragma
        Assert
          (if LME_Index + 1 > RME_Index - 1
             then
      ...
      begin
         Result :=
           Shift_Add (Result, Bits_To_Read, RME_Size, Value_Size - RME_Size);
      end;
      return Result;
   end U64_Extract;
      ...
      for I in reverse LME_Index + 1 .. RME_Index - 1 loop
         ...
         pragma
           Loop_Invariant
             (Fits_Into
                (Result,
                 Natural (RME_Index - I + 1) * Byte'Size - RME_Offset));
      end loop;
        ...
        Assert
          (if LME_Index + 1 <= RME_Index - 1
             then
               Fits_Into
                 (Result,
                  Natural (RME_Index - LME_Index) * Byte'Size - RME_Offset));
      pragma
        Assert
          (if LME_Index + 1 > RME_Index - 1
             then
   ...
   with
     Pre  =>
       ...
       and then Long_Integer ((Natural (Off) + Value_Size) - 1) / Byte'Size
                < Buffer (First .. Last)'Length,
     Post => Buffer'First = Buffer'Old'First and Buffer'Last = Buffer'Old'Last
   is
      RME_Index  : Index;
      LME_Index  : Index;
      RME_Offset : constant Natural := Natural (Off);
   ...
   begin
      ...
      if RME_Index = LME_Index then
         declare
            ...
            L_Bits      : constant U64 :=
              Mask_Lower (D, RME_Offset + Value_Size, Byte'Size);
            R_Bits      : constant U64 := Mask_Upper (D, RME_Offset);
            Bits_To_Add : constant U64 :=
              Left_Shift (Val, RME_Offset, Value_Size);
            Result      : constant U64 :=
              Add
                (L_Bits,
                 Add
                   (Bits_To_Add, R_Bits, RME_Offset + Value_Size, RME_Offset),
                 Byte'Size,
         ...
         case BO is
            when Low_Order_First  =>
               declare
                  L_Bits : constant U64 :=
                    Mask_Lower
                      (Byte'Pos (Buffer (LME_Index)), LME_Size, Byte'Size);
                  V_Bits : constant U64 := Mask_Upper (Val, LME_Size);
               begin
                  Buffer (LME_Index) :=
                    Byte'Val (Add (L_Bits, V_Bits, Byte'Size, LME_Size));
               end;
                  ...
                  RV :=
                    Right_Shift
                      (RV,
                       Byte'Size,
                       (Value_Size - LME_Size)
                       - Natural ((I - LME_Index) - 1) * Byte'Size);
                  pragma
                    Loop_Invariant
                      (Fits_Into
                         (RV,
                          (Value_Size - LME_Size)
                          - Natural (I - LME_Index) * Byte'Size));
               end loop;
               pragma
                 Assert
                   (RME_Size
                      = (Value_Size - LME_Size)
                        - Natural ((RME_Index - LME_Index) - 1) * Byte'Size);
               pragma Assert (Fits_Into (RV, RME_Size));
               declare
                  U_Value : constant U64 :=
                    Mask_Upper (Byte'Pos (Buffer (RME_Index)), RME_Offset);
                  R_Value : constant U64 :=
                    Left_Shift (RV, RME_Offset, RME_Size);
               begin
                  Buffer (RME_Index) :=
                    Byte'Val (Add (R_Value, U_Value, Byte'Size, RME_Offset));
               end;
            ...
            when High_Order_First =>
               ...
               declare
                  ...
                  V_Value : constant U64 :=
                    Left_Shift (V_Bits, RME_Offset, RME_Size);
               begin
                  ...
                  RV :=
                    Right_Shift
                      (RV,
                       Byte'Size,
                       (Value_Size - RME_Size)
                       - Natural ((RME_Index - I) - 1) * Byte'Size);
                  pragma
                    Loop_Invariant
                      (Fits_Into
                         (RV,
                          (Value_Size - RME_Size)
                          - Natural (RME_Index - I) * Byte'Size));
               end loop;
               pragma
                 Assert
                   (LME_Size
   ...
   function Extract
     (Buffer : Bytes;
      ...
      Off    : Offset;
      Size   : Positive;
      BO     : Byte_Order) return Base_Integer is
   begin
