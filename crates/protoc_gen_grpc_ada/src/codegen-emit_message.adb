...
package body Codegen.Emit_Message is
   ...
   begin
      ...
      --  A package body inherits every with clause of its spec, so the
      --  body only needs units the spec does not already pull in (else
      --  GNAT flags a redundant-with / unreferenced clause).  Unbounded
      --  and the message/enum field types come from the spec; the body
      --  adds just the wire codec.
      ...
      Append (Bod, ASCII.LF);
      --  use in the declarative part (not the context clause) so it
      --  resolves against the spec's with clause without a redundant
      --  body-level with; only emitted when a string/bytes field uses it.
      if Needs_Unbounded then
