...
package body RFLX.Control_Packet.Remaining_Length
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Continuation_1 =>
             Invalid (Ctx.Cursors (F_Value_1)),
          when F_Value_1 =>
             Invalid (Ctx.Cursors (F_Continuation_2)),
          when F_Continuation_2 =>
             Invalid (Ctx.Cursors (F_Value_2)),
          when F_Value_2 =>
             Invalid (Ctx.Cursors (F_Continuation_3)),
          when F_Continuation_3 =>
             Invalid (Ctx.Cursors (F_Value_3)),
          when F_Value_3 =>
             Invalid (Ctx.Cursors (F_Continuation_4)),
          when F_Continuation_4 =>
             Invalid (Ctx.Cursors (F_Value_4)),
          when F_Value_4 =>
             True);
   ...
   with
     Pre =>
       ...
       and RFLX.Control_Packet.Remaining_Length.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.Remaining_Length.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.Remaining_Length.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Control_Packet.Remaining_Length.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if
                              Fld in F_Value_1 | F_Value_2 | F_Value_3 | F_Value_4
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.Remaining_Length.Valid_Next (Ctx, Fld)
       and then Fld in F_Continuation_1 | F_Value_1 | F_Continuation_2 | F_Value_2 | F_Continuation_3 | F_Value_3 | F_Continuation_4 | F_Value_4
       and then RFLX.Control_Packet.Remaining_Length.Valid_Value (Fld, Val)
       and then RFLX.Control_Packet.Remaining_Length.Valid_Size (Ctx, Fld, RFLX.Control_Packet.Remaining_Length.Field_Size (Ctx, Fld))
       and then RFLX.Control_Packet.Remaining_Length.Available_Space (Ctx, Fld) >= RFLX.Control_Packet.Remaining_Length.Field_Size (Ctx, Fld)
       and then RFLX.Control_Packet.Remaining_Length.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Control_Packet.Remaining_Length.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Continuation_1 (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Continuation_1, To_Base_Integer (Val));
   end Set_Continuation_1;

   procedure Set_Value_1 (Ctx : in out Context; Val : RFLX.Control_Packet.Var_Byte_Value) is
   begin
      Set_Scalar (Ctx, F_Value_1, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Value_1;

   procedure Set_Continuation_2 (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Continuation_2, To_Base_Integer (Val));
   end Set_Continuation_2;

   procedure Set_Value_2 (Ctx : in out Context; Val : RFLX.Control_Packet.Var_Byte_Value) is
   begin
      Set_Scalar (Ctx, F_Value_2, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Value_2;

   procedure Set_Continuation_3 (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Continuation_3, To_Base_Integer (Val));
   end Set_Continuation_3;

   procedure Set_Value_3 (Ctx : in out Context; Val : RFLX.Control_Packet.Var_Byte_Value) is
   begin
      Set_Scalar (Ctx, F_Value_3, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Value_3;

   procedure Set_Continuation_4 (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Continuation_4, To_Base_Integer (Val));
   end Set_Continuation_4;

   procedure Set_Value_4 (Ctx : in out Context; Val : RFLX.Control_Packet.Var_Byte_Value) is
   begin
      Set_Scalar (Ctx, F_Value_4, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Value_4;
