...
package body RFLX.Puback.Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Puback.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Puback.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       ...
       and then RFLX.Puback.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Puback.Packet.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Puback.Packet.Valid_Value (Fld, Val)
       and then RFLX.Puback.Packet.Valid_Size (Ctx, Fld, RFLX.Puback.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Puback.Packet.Available_Space (Ctx, Fld) >= RFLX.Puback.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Puback.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Puback.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Puback.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Puback.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Puback.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Puback.To_Base_Integer (Val));
