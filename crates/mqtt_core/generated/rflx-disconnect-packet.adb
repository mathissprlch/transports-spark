...
package body RFLX.Disconnect.Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Disconnect.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Disconnect.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Remaining_Length then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Disconnect.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Disconnect.Packet.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Disconnect.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Packet_Type | F_Reserved | F_Remaining_Length
       and then RFLX.Disconnect.Packet.Valid_Value (Fld, Val)
       and then RFLX.Disconnect.Packet.Valid_Size (Ctx, Fld, RFLX.Disconnect.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Disconnect.Packet.Available_Space (Ctx, Fld) >= RFLX.Disconnect.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Disconnect.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Disconnect.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Disconnect.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Disconnect.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Disconnect.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Disconnect.To_Base_Integer (Val));
