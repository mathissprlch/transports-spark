     ...
     Post =>
       ...
       and then (if
                 ...
                 then
                    ...
                    and Session.Subscribing.FSM.Written_Last (Ctx, B_Inbound) = Session.Subscribing.FSM.Written_Last (Ctx, B_Inbound)'Old)
       and then (if
                    Ext_Buf /= B_Outgoing
                 then
                    Has_Buffer (Ctx, B_Outgoing) = Has_Buffer (Ctx, B_Outgoing)'Old
                    and Session.Subscribing.FSM.Written_Last (Ctx, B_Outgoing) = Session.Subscribing.FSM.Written_Last (Ctx, B_Outgoing)'Old)
...
private
   ...
   function Channel_Accessible (St : State; Chan : Channel) return Boolean is
     (case Chan is
          when C_Network =>
             (case St is
                 when S_Sending | S_Awaiting_Suback =>
                    True,
                 when others =>
                    False),
          when C_App_Pending =>
   ...
   function Written_Last (Ctx : Context; Ext_Buf : External_Buffer) return RFLX_Types.Bit_Length is
     (case Ext_Buf is
          ...
          when B_Outgoing =>
             Subscribe.Packet.Written_Last (Ctx.P.Outgoing_Ctx));
