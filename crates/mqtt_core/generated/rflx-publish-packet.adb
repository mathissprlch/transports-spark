...
package body RFLX.Publish.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Packet_Type =>
             Invalid (Ctx.Cursors (F_DUP)),
          when F_DUP =>
             Invalid (Ctx.Cursors (F_QoS)),
          when F_QoS =>
             Invalid (Ctx.Cursors (F_Retain)),
          when F_Retain =>
             Invalid (Ctx.Cursors (F_Remaining_Length)),
          when F_Remaining_Length =>
             Invalid (Ctx.Cursors (F_Topic_Name_Length)),
          when F_Topic_Name_Length =>
             Invalid (Ctx.Cursors (F_Topic_Name)),
          when F_Topic_Name =>
             Invalid (Ctx.Cursors (F_Packet_Identifier))
             and Invalid (Ctx.Cursors (F_Payload)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Publish.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Publish.Packet.Composite_Field (Fld)
   is
      ...
      Size : constant Positive := (case Fld is
          ...
          when F_Remaining_Length =>
             8,
          when F_Topic_Name_Length | F_Packet_Identifier =>
             16,
          when others =>
             Positive'Last);
   ...
   function Get_Topic_Name (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).Last);
   begin
   ...
   procedure Get_Topic_Name (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).Last);
   begin
   ...
   procedure Generic_Get_Topic_Name (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Name).Last);
   begin
      Process_Topic_Name (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Topic_Name;
   ...
   with
     Pre =>
       ...
       and then RFLX.Publish.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Publish.Packet.Available_Space (Ctx, Fld)
       and then (if RFLX.Publish.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Publish.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Packet_Type | F_DUP | F_QoS | F_Retain | F_Remaining_Length | F_Topic_Name_Length | F_Packet_Identifier
       and then RFLX.Publish.Packet.Valid_Value (Fld, Val)
       and then RFLX.Publish.Packet.Valid_Size (Ctx, Fld, RFLX.Publish.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Publish.Packet.Available_Space (Ctx, Fld) >= RFLX.Publish.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Publish.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Publish.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_DUP (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_DUP, To_Base_Integer (Val));
   end Set_DUP;

   procedure Set_QoS (Ctx : in out Context; Val : RFLX.Control_Packet.QoS_Level) is
   begin
      Set_Scalar (Ctx, F_QoS, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_QoS;

   procedure Set_Retain (Ctx : in out Context; Val : RFLX.Publish.Retain_Flag) is
   begin
      Set_Scalar (Ctx, F_Retain, RFLX.Publish.To_Base_Integer (Val));
   end Set_Retain;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Publish.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Publish.To_Base_Integer (Val));
   end Set_Remaining_Length;

   procedure Set_Topic_Name_Length (Ctx : in out Context; Val : RFLX.Control_Packet.String_Length) is
   begin
      Set_Scalar (Ctx, F_Topic_Name_Length, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Topic_Name_Length;

   procedure Set_Packet_Identifier (Ctx : in out Context; Val : RFLX.Control_Packet.Packet_Identifier) is
   begin
      Set_Scalar (Ctx, F_Packet_Identifier, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Packet_Identifier;

   procedure Set_Topic_Name_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Topic_Name, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Topic_Name_Empty;
   ...
   procedure Initialize_Topic_Name_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Publish.Packet.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Topic_Name) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Topic_Name)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Name);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Name) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Topic_Name);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Topic_Name) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Topic_Name_Private;

   procedure Initialize_Topic_Name (Ctx : in out Context) is
   begin
      Initialize_Topic_Name_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Topic_Name)));
   end Initialize_Topic_Name;
   ...
   procedure Set_Topic_Name (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Name));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Topic_Name_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Topic_Name)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Name)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Topic_Name))) = Data);
   end Set_Topic_Name;
   ...
   procedure Generic_Set_Topic_Name (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Name));
   begin
      if Length > 0 then
         Process_Topic_Name (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Publish.Packet.Valid_Length (Ctx, RFLX.Publish.Packet.F_Topic_Name, Length));
      Initialize_Topic_Name_Private (Ctx, Length);
   end Generic_Set_Topic_Name;
   ...
   begin
      ...
      pragma Assert (RFLX.Publish.Packet.Valid_Length (Ctx, RFLX.Publish.Packet.F_Payload, Length));
