...
package body RFLX.Subscribe.Subscription
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Topic_Filter_Length =>
             Invalid (Ctx.Cursors (F_Topic_Filter)),
          when F_Topic_Filter =>
             Invalid (Ctx.Cursors (F_Reserved_Sub_QoS)),
          when F_Reserved_Sub_QoS =>
             Invalid (Ctx.Cursors (F_Requested_QoS)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Subscribe.Subscription.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Subscribe.Subscription.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Requested_QoS then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Topic_Filter (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Topic_Filter;

   procedure Get_Topic_Filter (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).Last);
   begin
   ...
   procedure Generic_Get_Topic_Filter (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Topic_Filter).Last);
   begin
      Process_Topic_Filter (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Topic_Filter;
   ...
   with
     Pre =>
       RFLX.Subscribe.Subscription.Has_Buffer (Ctx)
       and then RFLX.Subscribe.Subscription.Valid_Next (Ctx, Fld)
       and then RFLX.Subscribe.Subscription.Valid_Value (Fld, Val)
       and then RFLX.Subscribe.Subscription.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Subscribe.Subscription.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Subscribe.Subscription.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Subscribe.Subscription.Valid_Next (Ctx, Fld)
       and then Fld in F_Topic_Filter_Length | F_Reserved_Sub_QoS | F_Requested_QoS
       and then RFLX.Subscribe.Subscription.Valid_Value (Fld, Val)
       and then RFLX.Subscribe.Subscription.Valid_Size (Ctx, Fld, RFLX.Subscribe.Subscription.Field_Size (Ctx, Fld))
       and then RFLX.Subscribe.Subscription.Available_Space (Ctx, Fld) >= RFLX.Subscribe.Subscription.Field_Size (Ctx, Fld)
       and then RFLX.Subscribe.Subscription.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Subscribe.Subscription.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Topic_Filter_Length (Ctx : in out Context; Val : RFLX.Control_Packet.String_Length) is
   begin
      Set_Scalar (Ctx, F_Topic_Filter_Length, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Topic_Filter_Length;

   procedure Set_Reserved_Sub_QoS (Ctx : in out Context; Val : RFLX.Subscribe.Reserved_Sub_QoS) is
   begin
      Set_Scalar (Ctx, F_Reserved_Sub_QoS, RFLX.Subscribe.To_Base_Integer (Val));
   end Set_Reserved_Sub_QoS;

   procedure Set_Requested_QoS (Ctx : in out Context; Val : RFLX.Control_Packet.QoS_Level) is
   begin
      Set_Scalar (Ctx, F_Requested_QoS, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Requested_QoS;

   procedure Set_Topic_Filter_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Topic_Filter, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Topic_Filter_Empty;

   procedure Initialize_Topic_Filter_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Subscribe.Subscription.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Topic_Filter) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Topic_Filter)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Filter);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Filter) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Topic_Filter);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Topic_Filter) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Topic_Filter_Private;

   procedure Initialize_Topic_Filter (Ctx : in out Context) is
   begin
      Initialize_Topic_Filter_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Topic_Filter)));
   end Initialize_Topic_Filter;

   procedure Set_Topic_Filter (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Filter));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Topic_Filter_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Topic_Filter)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Filter)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Topic_Filter))) = Data);
   end Set_Topic_Filter;

   procedure Generic_Set_Topic_Filter (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Topic_Filter));
   begin
      if Length > 0 then
         Process_Topic_Filter (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Subscribe.Subscription.Valid_Length (Ctx, RFLX.Subscribe.Subscription.F_Topic_Filter, Length));
      Initialize_Topic_Filter_Private (Ctx, Length);
   end Generic_Set_Topic_Filter;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Topic_Filter_Length := Get_Topic_Filter_Length (Ctx);
      Struct.Topic_Filter := (others => 0);
      Get_Topic_Filter (Ctx, Struct.Topic_Filter (Struct.Topic_Filter'First .. Struct.Topic_Filter'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Topic_Filter)) + 1) - 2));
      Struct.Reserved_Sub_QoS := Get_Reserved_Sub_QoS (Ctx);
      Struct.Requested_QoS := Get_Requested_QoS (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Topic_Filter_Length (Ctx, Struct.Topic_Filter_Length);
      Set_Topic_Filter (Ctx, Struct.Topic_Filter (Struct.Topic_Filter'First .. Struct.Topic_Filter'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Topic_Filter_Length) * 8) + 1) - 2));
      Set_Reserved_Sub_QoS (Ctx, Struct.Reserved_Sub_QoS);
      Set_Requested_QoS (Ctx, Struct.Requested_QoS);
   end To_Context;
