...
package body RFLX.Subscribe.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Packet_Identifier =>
             Invalid (Ctx.Cursors (F_Subscriptions)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Subscribe.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Subscribe.Packet.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Subscriptions then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Subscribe.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Subscribe.Packet.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Subscribe.Packet.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Subscribe.Packet.Valid_Value (Fld, Val)
       and then RFLX.Subscribe.Packet.Valid_Size (Ctx, Fld, RFLX.Subscribe.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Subscribe.Packet.Available_Space (Ctx, Fld) >= RFLX.Subscribe.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Subscribe.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Subscribe.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Subscribe.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Subscribe.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Subscribe.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Subscribe.To_Base_Integer (Val));
   ...
   procedure Set_Subscriptions (Ctx : in out Context; Seq_Ctx : RFLX.Subscribe.Subscription_List.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.Subscribe.Subscription_List.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Subscriptions, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.Subscribe.Subscription_List.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Subscriptions;

   procedure Initialize_Subscriptions_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Subscribe.Packet.Has_Buffer (Ctx)
       and then RFLX.Subscribe.Packet.Valid_Next (Ctx, RFLX.Subscribe.Packet.F_Subscriptions)
       and then RFLX.Subscribe.Packet.Valid_Length (Ctx, RFLX.Subscribe.Packet.F_Subscriptions, Length)
       and then RFLX_Types.To_Length (RFLX.Subscribe.Packet.Available_Space (Ctx, RFLX.Subscribe.Packet.F_Subscriptions)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Subscriptions)
       and then Field_Size (Ctx, F_Subscriptions) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Subscriptions)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Subscriptions);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Subscriptions) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Subscriptions);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Subscriptions) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Subscriptions_Private;

   procedure Initialize_Subscriptions (Ctx : in out Context) is
   begin
      Initialize_Subscriptions_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Subscriptions)));
   end Initialize_Subscriptions;

   procedure Switch_To_Subscriptions (Ctx : in out Context; Seq_Ctx : out RFLX.Subscribe.Subscription_List.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Subscriptions);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Subscriptions);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Subscriptions) then
         Reset_Dependent_Fields (Ctx, F_Subscriptions);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.Subscribe.Subscription_List.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Subscriptions;

   procedure Update_Subscriptions (Ctx : in out Context; Seq_Ctx : in out RFLX.Subscribe.Subscription_List.Context) is
      Valid_Sequence : constant Boolean := RFLX.Subscribe.Packet.Complete_Subscriptions (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.Subscribe.Subscription_List.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Subscriptions) := (State => S_Valid, First => Ctx.Cursors (F_Subscriptions).First, Last => Ctx.Cursors (F_Subscriptions).Last, Value => Ctx.Cursors (F_Subscriptions).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Subscriptions);
         Ctx.Cursors (F_Subscriptions) := (others => <>);
      end if;
