...
package body RFLX.Connack.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Remaining_Length =>
             Invalid (Ctx.Cursors (F_Reserved_Ack_Flags)),
          when F_Reserved_Ack_Flags =>
             Invalid (Ctx.Cursors (F_Session_Present)),
          when F_Session_Present =>
             Invalid (Ctx.Cursors (F_Return_Code)),
          when F_Return_Code =>
             True);
   ...
   with
     Pre =>
       ...
       and RFLX.Connack.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Connack.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Return_Code then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Connack.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Connack.Packet.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Connack.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Packet_Type | F_Reserved | F_Remaining_Length | F_Reserved_Ack_Flags | F_Session_Present | F_Return_Code
       and then RFLX.Connack.Packet.Valid_Value (Fld, Val)
       and then RFLX.Connack.Packet.Valid_Size (Ctx, Fld, RFLX.Connack.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Connack.Packet.Available_Space (Ctx, Fld) >= RFLX.Connack.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Connack.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Connack.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Connack.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Connack.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Connack.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Connack.To_Base_Integer (Val));
   end Set_Remaining_Length;

   procedure Set_Reserved_Ack_Flags (Ctx : in out Context; Val : RFLX.Connack.Reserved_Ack_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved_Ack_Flags, RFLX.Connack.To_Base_Integer (Val));
   end Set_Reserved_Ack_Flags;

   procedure Set_Session_Present (Ctx : in out Context; Val : Boolean) is
   begin
      Set_Scalar (Ctx, F_Session_Present, To_Base_Integer (Val));
   end Set_Session_Present;

   procedure Set_Return_Code (Ctx : in out Context; Val : RFLX.Connack.Connect_Return_Code) is
   begin
      Set_Scalar (Ctx, F_Return_Code, RFLX.Connack.To_Base_Integer (Val));
   end Set_Return_Code;
   ...
   begin
      ...
      Struct.Reserved_Ack_Flags := Get_Reserved_Ack_Flags (Ctx);
      Struct.Session_Present := Get_Session_Present (Ctx);
      Struct.Return_Code := Get_Return_Code (Ctx);
   end To_Structure;
   ...
   begin
      ...
      Set_Reserved_Ack_Flags (Ctx, Struct.Reserved_Ack_Flags);
      Set_Session_Present (Ctx, Struct.Session_Present);
      Set_Return_Code (Ctx, Struct.Return_Code);
   end To_Context;
