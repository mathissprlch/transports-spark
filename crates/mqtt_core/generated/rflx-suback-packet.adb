...
package body RFLX.Suback.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Packet_Identifier =>
             Invalid (Ctx.Cursors (F_Return_Codes)),
          when F_Return_Codes =>
             True);
   ...
   with
     Pre =>
       ...
       and then RFLX.Suback.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Suback.Packet.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Return_Codes then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Suback.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Suback.Packet.Available_Space (Ctx, Fld)
       and then (if RFLX.Suback.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Suback.Packet.Valid_Value (Fld, Val)
       and then RFLX.Suback.Packet.Valid_Size (Ctx, Fld, RFLX.Suback.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Suback.Packet.Available_Space (Ctx, Fld) >= RFLX.Suback.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Suback.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Suback.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Suback.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Suback.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Suback.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Suback.To_Base_Integer (Val));
   ...
   procedure Set_Return_Codes (Ctx : in out Context; Seq_Ctx : RFLX.Suback.Return_Code_List.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.Suback.Return_Code_List.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Return_Codes, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.Suback.Return_Code_List.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Return_Codes;

   procedure Initialize_Return_Codes_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Suback.Packet.Has_Buffer (Ctx)
       and then RFLX.Suback.Packet.Valid_Next (Ctx, RFLX.Suback.Packet.F_Return_Codes)
       and then RFLX.Suback.Packet.Valid_Length (Ctx, RFLX.Suback.Packet.F_Return_Codes, Length)
       and then RFLX_Types.To_Length (RFLX.Suback.Packet.Available_Space (Ctx, RFLX.Suback.Packet.F_Return_Codes)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Return_Codes)
       and then Field_Size (Ctx, F_Return_Codes) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Return_Codes)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Return_Codes);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Return_Codes) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Return_Codes);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Return_Codes) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Return_Codes_Private;

   procedure Initialize_Return_Codes (Ctx : in out Context) is
   begin
      Initialize_Return_Codes_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Return_Codes)));
   end Initialize_Return_Codes;

   procedure Switch_To_Return_Codes (Ctx : in out Context; Seq_Ctx : out RFLX.Suback.Return_Code_List.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Return_Codes);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Return_Codes);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Return_Codes) then
         Reset_Dependent_Fields (Ctx, F_Return_Codes);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.Suback.Return_Code_List.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Return_Codes;

   procedure Update_Return_Codes (Ctx : in out Context; Seq_Ctx : in out RFLX.Suback.Return_Code_List.Context) is
      Valid_Sequence : constant Boolean := RFLX.Suback.Packet.Complete_Return_Codes (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.Suback.Return_Code_List.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Return_Codes) := (State => S_Valid, First => Ctx.Cursors (F_Return_Codes).First, Last => Ctx.Cursors (F_Return_Codes).Last, Value => Ctx.Cursors (F_Return_Codes).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Return_Codes);
         Ctx.Cursors (F_Return_Codes) := (others => <>);
      end if;
   end Update_Return_Codes;
