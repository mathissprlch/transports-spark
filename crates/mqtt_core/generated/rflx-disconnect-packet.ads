...
package RFLX.Disconnect.Packet
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Packet_Type, F_Reserved, F_Remaining_Length, F_Final);

   subtype Field is Virtual_Field range F_Packet_Type .. F_Remaining_Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Disconnect.Packet.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Well_Formed_Message (Ctx)
       and then RFLX.Disconnect.Packet.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then Offset < RFLX.Disconnect.Packet.Buffer_Length (Ctx)
       and then Pre (RFLX.Disconnect.Packet.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Disconnect.Packet.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Valid_Value (Fld, Val)
       and then RFLX.Disconnect.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Disconnect.Packet.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       RFLX.Disconnect.Packet.Valid (Ctx, RFLX.Disconnect.Packet.F_Packet_Type);

   function Get_Reserved (Ctx : Context) return RFLX.Disconnect.Reserved_Flags
   with
     Pre =>
       RFLX.Disconnect.Packet.Valid (Ctx, RFLX.Disconnect.Packet.F_Reserved);

   function Get_Remaining_Length (Ctx : Context) return RFLX.Disconnect.Remaining_Length
   with
     Pre =>
       RFLX.Disconnect.Packet.Valid (Ctx, RFLX.Disconnect.Packet.F_Remaining_Length);
   ...
   with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Valid_Next (Ctx, RFLX.Disconnect.Packet.F_Packet_Type)
       and then RFLX.Control_Packet.Valid_Packet_Type (RFLX.Control_Packet.To_Base_Integer (Val))
       and then RFLX.Disconnect.Packet.Available_Space (Ctx, RFLX.Disconnect.Packet.F_Packet_Type) >= RFLX.Disconnect.Packet.Field_Size (Ctx, RFLX.Disconnect.Packet.F_Packet_Type)
       and then RFLX.Disconnect.Packet.Field_Condition (Ctx, RFLX.Disconnect.Packet.F_Packet_Type, RFLX.Control_Packet.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (if
               RFLX_Types.Base_Integer (To_Base_Integer (Get_Packet_Type (Ctx))) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Control_Packet.DISCONNECT))
            then
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Disconnect.Reserved_Flags)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Valid_Next (Ctx, RFLX.Disconnect.Packet.F_Reserved)
       and then RFLX.Disconnect.Valid_Reserved_Flags (RFLX.Disconnect.To_Base_Integer (Val))
       and then RFLX.Disconnect.Packet.Available_Space (Ctx, RFLX.Disconnect.Packet.F_Reserved) >= RFLX.Disconnect.Packet.Field_Size (Ctx, RFLX.Disconnect.Packet.F_Reserved)
       and then RFLX.Disconnect.Packet.Field_Condition (Ctx, RFLX.Disconnect.Packet.F_Reserved, RFLX.Disconnect.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Disconnect.Remaining_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Valid_Next (Ctx, RFLX.Disconnect.Packet.F_Remaining_Length)
       and then RFLX.Disconnect.Valid_Remaining_Length (RFLX.Disconnect.To_Base_Integer (Val))
       and then RFLX.Disconnect.Packet.Available_Space (Ctx, RFLX.Disconnect.Packet.F_Remaining_Length) >= RFLX.Disconnect.Packet.Field_Size (Ctx, RFLX.Disconnect.Packet.F_Remaining_Length)
       and then RFLX.Disconnect.Packet.Field_Condition (Ctx, RFLX.Disconnect.Packet.F_Remaining_Length, RFLX.Disconnect.To_Base_Integer (Val)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Remaining_Length)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Remaining_Length))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   type Structure is
      record
         ...
         Reserved : RFLX.Disconnect.Reserved_Flags;
         Remaining_Length : RFLX.Disconnect.Remaining_Length;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Disconnect.Packet.Has_Buffer (Ctx)
       and then RFLX.Disconnect.Packet.Valid_Structure (Struct)
       and then RFLX.Disconnect.Packet.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
      ...
      and then (if
                ...
                then
                   (Valid (Cursors (F_Packet_Type))
                    and then RFLX_Types.Base_Integer (Cursors (F_Packet_Type).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Control_Packet.DISCONNECT))))
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Disconnect.Packet.Field_First (Ctx, RFLX.Disconnect.Packet.F_Packet_Type) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          ...
          when F_Reserved =>
             RFLX.Disconnect.Valid_Reserved_Flags (Val),
          when F_Remaining_Length =>
             RFLX.Disconnect.Valid_Remaining_Length (Val));

   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Packet_Type =>
             Val = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Control_Packet.DISCONNECT)),
   ...
   function Get_Reserved (Ctx : Context) return RFLX.Disconnect.Reserved_Flags is
     (To_Actual (Ctx.Cursors (F_Reserved).Value));

   function Get_Remaining_Length (Ctx : Context) return RFLX.Disconnect.Remaining_Length is
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (To_Base_Integer (Struct.Packet_Type) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Control_Packet.DISCONNECT)));
