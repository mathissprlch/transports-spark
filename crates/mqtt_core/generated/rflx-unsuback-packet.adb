...
package body RFLX.Unsuback.Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Unsuback.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsuback.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Packet_Identifier then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsuback.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Unsuback.Packet.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsuback.Packet.Valid_Value (Fld, Val)
       and then RFLX.Unsuback.Packet.Valid_Size (Ctx, Fld, RFLX.Unsuback.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Unsuback.Packet.Available_Space (Ctx, Fld) >= RFLX.Unsuback.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Unsuback.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Unsuback.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Unsuback.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Unsuback.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Unsuback.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Unsuback.To_Base_Integer (Val));
   ...
   begin
      ...
      Struct.Packet_Identifier := Get_Packet_Identifier (Ctx);
   end To_Structure;
   ...
   begin
      ...
      Set_Packet_Identifier (Ctx, Struct.Packet_Identifier);
   end To_Context;
