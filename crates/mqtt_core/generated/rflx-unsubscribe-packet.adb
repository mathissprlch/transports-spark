...
package body RFLX.Unsubscribe.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          ...
          when F_Packet_Identifier =>
             Invalid (Ctx.Cursors (F_Topic_Filters)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsubscribe.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Unsubscribe.Packet.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Topic_Filters then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsubscribe.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Unsubscribe.Packet.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Unsubscribe.Packet.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Unsubscribe.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Packet_Type | F_Reserved | F_Remaining_Length | F_Packet_Identifier
       and then RFLX.Unsubscribe.Packet.Valid_Value (Fld, Val)
       and then RFLX.Unsubscribe.Packet.Valid_Size (Ctx, Fld, RFLX.Unsubscribe.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Unsubscribe.Packet.Available_Space (Ctx, Fld) >= RFLX.Unsubscribe.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Unsubscribe.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Unsubscribe.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Unsubscribe.Reserved_Flags) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Unsubscribe.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Unsubscribe.Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Unsubscribe.To_Base_Integer (Val));
   ...
   procedure Set_Topic_Filters (Ctx : in out Context; Seq_Ctx : RFLX.Unsubscribe.Topic_Filter_List.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.Unsubscribe.Topic_Filter_List.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Topic_Filters, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.Unsubscribe.Topic_Filter_List.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Topic_Filters;

   procedure Initialize_Topic_Filters_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Unsubscribe.Packet.Has_Buffer (Ctx)
       and then RFLX.Unsubscribe.Packet.Valid_Next (Ctx, RFLX.Unsubscribe.Packet.F_Topic_Filters)
       and then RFLX.Unsubscribe.Packet.Valid_Length (Ctx, RFLX.Unsubscribe.Packet.F_Topic_Filters, Length)
       and then RFLX_Types.To_Length (RFLX.Unsubscribe.Packet.Available_Space (Ctx, RFLX.Unsubscribe.Packet.F_Topic_Filters)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Topic_Filters)
       and then Field_Size (Ctx, F_Topic_Filters) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Topic_Filters)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Filters);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Filters) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Topic_Filters);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Topic_Filters) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Topic_Filters_Private;

   procedure Initialize_Topic_Filters (Ctx : in out Context) is
   begin
      Initialize_Topic_Filters_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Topic_Filters)));
   end Initialize_Topic_Filters;

   procedure Switch_To_Topic_Filters (Ctx : in out Context; Seq_Ctx : out RFLX.Unsubscribe.Topic_Filter_List.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Topic_Filters);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Topic_Filters);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Topic_Filters) then
         Reset_Dependent_Fields (Ctx, F_Topic_Filters);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.Unsubscribe.Topic_Filter_List.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Topic_Filters;

   procedure Update_Topic_Filters (Ctx : in out Context; Seq_Ctx : in out RFLX.Unsubscribe.Topic_Filter_List.Context) is
      Valid_Sequence : constant Boolean := RFLX.Unsubscribe.Packet.Complete_Topic_Filters (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.Unsubscribe.Topic_Filter_List.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Topic_Filters) := (State => S_Valid, First => Ctx.Cursors (F_Topic_Filters).First, Last => Ctx.Cursors (F_Topic_Filters).Last, Value => Ctx.Cursors (F_Topic_Filters).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Topic_Filters);
         Ctx.Cursors (F_Topic_Filters) := (others => <>);
      end if;
   end Update_Topic_Filters;
