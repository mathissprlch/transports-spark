...
package RFLX.Session.Publish_Qos2.FSM
with
  SPARK_Mode
is
   ...
   type State is (S_Loading, S_Sending, S_Awaiting_Pubrec, S_Forwarding_Inbound_Publish, S_Forwarding_Pubrec, S_Awaiting_Pubcomp, S_Forwarding_Inbound_Publish_2, S_Forwarding_Pubcomp, S_Final);
     ...
     Post =>
       ...
       and then (if
                 ...
                 then
                    ...
                    and Session.Publish_Qos2.FSM.Written_Last (Ctx, B_Inbound) = Session.Publish_Qos2.FSM.Written_Last (Ctx, B_Inbound)'Old)
       and then (if
                    Ext_Buf /= B_Outgoing
                 then
                    Has_Buffer (Ctx, B_Outgoing) = Has_Buffer (Ctx, B_Outgoing)'Old
                    and Session.Publish_Qos2.FSM.Written_Last (Ctx, B_Outgoing) = Session.Publish_Qos2.FSM.Written_Last (Ctx, B_Outgoing)'Old)
...
private
   ...
   function Channel_Accessible (St : State; Chan : Channel) return Boolean is
     (case Chan is
          when C_Network =>
             (case St is
                 when S_Sending | S_Awaiting_Pubrec | S_Awaiting_Pubcomp =>
                    True,
                 when others =>
                    False),
          when C_App_Pending =>
