...
package body RFLX.Control_Packet.Incoming_Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.Incoming_Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Control_Packet.Incoming_Packet.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.Control_Packet.Incoming_Packet.Has_Buffer (Ctx)
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Value (Fld, Val)
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Control_Packet.Incoming_Packet.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Control_Packet.Incoming_Packet.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Packet_Type | F_Flags | F_Remaining_Length
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Value (Fld, Val)
       and then RFLX.Control_Packet.Incoming_Packet.Valid_Size (Ctx, Fld, RFLX.Control_Packet.Incoming_Packet.Field_Size (Ctx, Fld))
       and then RFLX.Control_Packet.Incoming_Packet.Available_Space (Ctx, Fld) >= RFLX.Control_Packet.Incoming_Packet.Field_Size (Ctx, Fld)
       and then RFLX.Control_Packet.Incoming_Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Control_Packet.Incoming_Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Flags (Ctx : in out Context; Val : RFLX.Control_Packet.Header_Flags) is
   begin
      Set_Scalar (Ctx, F_Flags, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Flags;

   procedure Set_Remaining_Length (Ctx : in out Context; Val : RFLX.Control_Packet.Single_Byte_Remaining_Length) is
   begin
      Set_Scalar (Ctx, F_Remaining_Length, RFLX.Control_Packet.To_Base_Integer (Val));
   ...
   begin
      ...
      pragma Assert (RFLX.Control_Packet.Incoming_Packet.Valid_Length (Ctx, RFLX.Control_Packet.Incoming_Packet.F_Payload, Length));
      Initialize_Payload_Private (Ctx, Length);
   end Generic_Set_Payload;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Packet_Type := Get_Packet_Type (Ctx);
      Struct.Flags := Get_Flags (Ctx);
      Struct.Remaining_Length := Get_Remaining_Length (Ctx);
   ...
   begin
      Reset (Ctx);
      Set_Packet_Type (Ctx, Struct.Packet_Type);
      Set_Flags (Ctx, Struct.Flags);
      Set_Remaining_Length (Ctx, Struct.Remaining_Length);
      Set_Payload (Ctx, Struct.Payload (Struct.Payload'First .. Struct.Payload'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Remaining_Length) * 8) + 1) - 2));
   end To_Context;
