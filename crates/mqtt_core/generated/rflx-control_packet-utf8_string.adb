...
package body RFLX.Control_Packet.UTF8_String
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.UTF8_String.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Control_Packet.UTF8_String.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.Control_Packet.UTF8_String.Has_Buffer (Ctx)
       and then RFLX.Control_Packet.UTF8_String.Valid_Next (Ctx, Fld)
       and then RFLX.Control_Packet.UTF8_String.Valid_Value (Fld, Val)
       and then RFLX.Control_Packet.UTF8_String.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Control_Packet.UTF8_String.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Control_Packet.UTF8_String.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Control_Packet.UTF8_String.Valid_Value (Fld, Val)
       and then RFLX.Control_Packet.UTF8_String.Valid_Size (Ctx, Fld, RFLX.Control_Packet.UTF8_String.Field_Size (Ctx, Fld))
       and then RFLX.Control_Packet.UTF8_String.Available_Space (Ctx, Fld) >= RFLX.Control_Packet.UTF8_String.Field_Size (Ctx, Fld)
       and then RFLX.Control_Packet.UTF8_String.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Control_Packet.UTF8_String.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.Control_Packet.String_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.Control_Packet.To_Base_Integer (Val));
   end Set_Length;
   ...
   begin
      ...
      pragma Assert (RFLX.Control_Packet.UTF8_String.Valid_Length (Ctx, RFLX.Control_Packet.UTF8_String.F_Data, Length));
