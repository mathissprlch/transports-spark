...
package RFLX.Session.Connect_Handshake.FSM
with
  SPARK_Mode
is
   ...
   type State is (S_Loading, S_Sending, S_Awaiting_Connack, S_Forwarding_Connack, S_Final);
     ...
     Post =>
       ...
       and then (if
                 ...
                 then
                    ...
                    and Session.Connect_Handshake.FSM.Written_Last (Ctx, B_Inbound) = Session.Connect_Handshake.FSM.Written_Last (Ctx, B_Inbound)'Old)
       and then (if
                    Ext_Buf /= B_Outgoing
                 then
                    Has_Buffer (Ctx, B_Outgoing) = Has_Buffer (Ctx, B_Outgoing)'Old
                    and Session.Connect_Handshake.FSM.Written_Last (Ctx, B_Outgoing) = Session.Connect_Handshake.FSM.Written_Last (Ctx, B_Outgoing)'Old)
...
private
   ...
   function Buffers_Initialized (Ctx : Context) return Boolean is
     (Connect.Packet.Has_Buffer (Ctx.P.Outgoing_Ctx)
   ...
   function Channel_Accessible (St : State; Chan : Channel) return Boolean is
     (case Chan is
          when C_Network =>
             (case St is
                 when S_Sending | S_Awaiting_Connack =>
                    True,
                 when others =>
                    False),
          when C_App_Pending =>
   ...
   function Written_Last (Ctx : Context; Ext_Buf : External_Buffer) return RFLX_Types.Bit_Length is
     (case Ext_Buf is
          ...
          when B_Outgoing =>
             Connect.Packet.Written_Last (Ctx.P.Outgoing_Ctx));
     ...
     (case Chan is
          when C_Network =>
             (case Ctx.P.Next_State is
                 when S_Sending =>
                    Connect.Packet.Well_Formed_Message (Ctx.P.Outgoing_Ctx)
                    and Connect.Packet.Byte_Size (Ctx.P.Outgoing_Ctx) > 0,
                 when others =>
                    False),
          when C_App_Pending =>
             (case Ctx.P.Next_State is
   ...
   function Read_Buffer_Size (Ctx : Context; Chan : Channel) return RFLX_Types.Length is
     (case Chan is
          when C_Network =>
             (case Ctx.P.Next_State is
                 when S_Sending =>
                    Connect.Packet.Byte_Size (Ctx.P.Outgoing_Ctx),
                 when others =>
   ...
   function Write_Buffer_Size (Ctx : Context; Chan : Channel) return RFLX_Types.Length is
     (case Chan is
          ...
          when C_App_Outbox =>
             (case Ctx.P.Next_State is
                 when S_Loading =>
                    Connect.Packet.Buffer_Length (Ctx.P.Outgoing_Ctx),
                 when others =>
                    RFLX_Types.Unreachable));
