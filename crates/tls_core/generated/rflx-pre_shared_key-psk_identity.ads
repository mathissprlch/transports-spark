...
package RFLX.Pre_Shared_Key.Psk_Identity
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Identity_Len, F_Identity, F_Obfuscated_Ticket_Age, F_Final);

   subtype Field is Virtual_Field range F_Identity_Len .. F_Obfuscated_Ticket_Age;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Well_Formed_Message (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then Offset < RFLX.Pre_Shared_Key.Psk_Identity.Buffer_Length (Ctx)
       and then Pre (RFLX.Pre_Shared_Key.Psk_Identity.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Pre_Shared_Key.Psk_Identity.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Identity =>
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, Fld);
   ...
   function Get_Identity_Len (Ctx : Context) return RFLX.Pre_Shared_Key.Identity_Length
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Valid (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len);

   function Get_Obfuscated_Ticket_Age (Ctx : Context) return RFLX.Pre_Shared_Key.Ticket_Age
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Valid (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Obfuscated_Ticket_Age);
   ...
   with
     ...
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Well_Formed (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Well_Formed (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)),
     Post =>
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and RFLX.Pre_Shared_Key.Psk_Identity.Present (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity);
   ...
   procedure Set_Identity_Len (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Identity_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len)
       and then RFLX.Pre_Shared_Key.Valid_Identity_Length (RFLX.Pre_Shared_Key.To_Base_Integer (Val))
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Field_Condition (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Identity_Len)
       and Get_Identity_Len (Ctx) = Val
       ...
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Identity_Len) = Valid_Next (Ctx, F_Identity_Len)'Old
       and Field_First (Ctx, F_Identity_Len) = Field_First (Ctx, F_Identity_Len)'Old;

   procedure Set_Obfuscated_Ticket_Age (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Ticket_Age)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Obfuscated_Ticket_Age)
       and then RFLX.Pre_Shared_Key.Valid_Ticket_Age (RFLX.Pre_Shared_Key.To_Base_Integer (Val))
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Obfuscated_Ticket_Age) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Obfuscated_Ticket_Age)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Field_Condition (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Obfuscated_Ticket_Age),
     Post =>
       Has_Buffer (Ctx)
       ...
       and Get_Identity_Len (Ctx) = Get_Identity_Len (Ctx)'Old
       and Field_First (Ctx, F_Obfuscated_Ticket_Age) = Field_First (Ctx, F_Obfuscated_Ticket_Age)'Old
       and (for all F in Field range F_Identity_Len .. F_Identity =>
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity),
     Post =>
       Has_Buffer (Ctx)
       ...
       and then Get_Identity_Len (Ctx) = Get_Identity_Len (Ctx)'Old
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity, Data'Length)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.Pre_Shared_Key.Psk_Identity.Field_Condition (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity, Length)
       and then RFLX_Types.To_Length (RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity)) >= Length
   ...
   type Structure is
      record
         Identity_Len : RFLX.Pre_Shared_Key.Identity_Length;
         Identity : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 65534);
         Obfuscated_Ticket_Age : RFLX.Pre_Shared_Key.Ticket_Age;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Structure (Struct)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Identity_Len (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Identity_Len)) then True)
      and then (if Well_Formed (Cursors (F_Identity)) then Valid (Cursors (F_Identity_Len)))
   ...
   function Field_Size_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Length'Base is
     (case Fld is
          when F_Identity_Len =>
             16,
          when F_Identity =>
             RFLX_Types.Bit_Length (Cursors (F_Identity_Len).Value) * 8,
          when F_Obfuscated_Ticket_Age =>
             32)
   with
     Pre =>
   ...
   function Field_First_Identity_Len (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Identity_Len);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Identity_Len =>
             Field_First_Identity_Len (Cursors, First, Verified_Last, Written_Last),
          when F_Identity =>
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Identity_Len))
                 then
                    (Cursors (F_Identity_Len).Last - Cursors (F_Identity_Len).First + 1 = 16
                     and then Cursors (F_Identity_Len).First = First))
                and then (if
                             Well_Formed (Cursors (F_Identity))
                          then
                             (Cursors (F_Identity).Last - Cursors (F_Identity).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Identity_Len).Value) * 8
                              and then Cursors (F_Identity).First = Cursors (F_Identity_Len).Last + 1))
                and then (if
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Identity_Len)
      and then RFLX.Pre_Shared_Key.Psk_Identity.Field_First (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity_Len) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Identity_Len) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Identity_Len =>
             RFLX.Pre_Shared_Key.Valid_Identity_Length (Val),
          when F_Identity =>
             True,
          when F_Obfuscated_Ticket_Age =>
             RFLX.Pre_Shared_Key.Valid_Ticket_Age (Val));

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Identity_Len | F_Identity | F_Obfuscated_Ticket_Age =>
             True);
   ...
   function Get_Identity_Len (Ctx : Context) return RFLX.Pre_Shared_Key.Identity_Length is
     (To_Actual (Ctx.Cursors (F_Identity_Len).Value));

   function Get_Obfuscated_Ticket_Age (Ctx : Context) return RFLX.Pre_Shared_Key.Ticket_Age is
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Identity_Len) * 8 + 48);

   function Field_Size_Identity_Len (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Identity (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Identity_Len) * 8);

   function Field_Size_Obfuscated_Ticket_Age (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.Pre_Shared_Key.Psk_Identity;
