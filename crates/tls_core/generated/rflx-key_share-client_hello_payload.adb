...
package body RFLX.Key_Share.Client_Hello_Payload
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Client_Shares_Len =>
             Invalid (Ctx.Cursors (F_Client_Shares)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Key_Share.Client_Hello_Payload.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Key_Share.Client_Hello_Payload.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Client_Shares then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Client_Shares (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Client_Shares;

   procedure Get_Client_Shares (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).Last);
   begin
   ...
   procedure Generic_Get_Client_Shares (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Client_Shares).Last);
   begin
      Process_Client_Shares (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Client_Shares;
   ...
   with
     Pre =>
       RFLX.Key_Share.Client_Hello_Payload.Has_Buffer (Ctx)
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Next (Ctx, Fld)
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Value (Fld, Val)
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Key_Share.Client_Hello_Payload.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Key_Share.Client_Hello_Payload.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Next (Ctx, Fld)
       and then Fld in F_Client_Shares_Len
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Value (Fld, Val)
       and then RFLX.Key_Share.Client_Hello_Payload.Valid_Size (Ctx, Fld, RFLX.Key_Share.Client_Hello_Payload.Field_Size (Ctx, Fld))
       and then RFLX.Key_Share.Client_Hello_Payload.Available_Space (Ctx, Fld) >= RFLX.Key_Share.Client_Hello_Payload.Field_Size (Ctx, Fld)
       and then RFLX.Key_Share.Client_Hello_Payload.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Key_Share.Client_Hello_Payload.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Client_Shares_Len (Ctx : in out Context; Val : RFLX.Key_Share.Client_Shares_Length) is
   begin
      Set_Scalar (Ctx, F_Client_Shares_Len, RFLX.Key_Share.To_Base_Integer (Val));
   end Set_Client_Shares_Len;

   procedure Set_Client_Shares_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Client_Shares, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Client_Shares_Empty;

   procedure Initialize_Client_Shares_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Key_Share.Client_Hello_Payload.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Client_Shares) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Client_Shares)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Client_Shares);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Client_Shares) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Client_Shares);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Client_Shares) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Client_Shares_Private;

   procedure Initialize_Client_Shares (Ctx : in out Context) is
   begin
      Initialize_Client_Shares_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Client_Shares)));
   end Initialize_Client_Shares;

   procedure Set_Client_Shares (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Client_Shares));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Client_Shares_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Client_Shares)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Client_Shares)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Client_Shares))) = Data);
   end Set_Client_Shares;

   procedure Generic_Set_Client_Shares (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Client_Shares));
   begin
      if Length > 0 then
         Process_Client_Shares (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Key_Share.Client_Hello_Payload.Valid_Length (Ctx, RFLX.Key_Share.Client_Hello_Payload.F_Client_Shares, Length));
      Initialize_Client_Shares_Private (Ctx, Length);
   end Generic_Set_Client_Shares;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Client_Shares_Len := Get_Client_Shares_Len (Ctx);
      Struct.Client_Shares := (others => 0);
      Get_Client_Shares (Ctx, Struct.Client_Shares (Struct.Client_Shares'First .. Struct.Client_Shares'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Client_Shares)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Client_Shares_Len (Ctx, Struct.Client_Shares_Len);
      Set_Client_Shares (Ctx, Struct.Client_Shares (Struct.Client_Shares'First .. Struct.Client_Shares'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Client_Shares_Len) * 8) + 1) - 2));
   end To_Context;
