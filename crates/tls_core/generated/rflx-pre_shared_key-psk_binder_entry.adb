...
package body RFLX.Pre_Shared_Key.Psk_Binder_Entry
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Pre_Shared_Key.Psk_Binder_Entry.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Binder then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Binder (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Binder;

   procedure Get_Binder (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).Last);
   begin
   ...
   procedure Generic_Get_Binder (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Binder).Last);
   begin
      Process_Binder (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Binder;
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Value (Fld, Val)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Pre_Shared_Key.Psk_Binder_Entry.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, Fld)
       and then Fld in F_Binder_Len
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Value (Fld, Val)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Size (Ctx, Fld, RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, Fld))
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, Fld) >= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Binder_Len (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Binder_Length) is
   begin
      Set_Scalar (Ctx, F_Binder_Len, RFLX.Pre_Shared_Key.To_Base_Integer (Val));
   end Set_Binder_Len;

   procedure Initialize_Binder_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Binder) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Binder)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Binder);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Binder) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      ...
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Binder) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Binder_Private;

   procedure Initialize_Binder (Ctx : in out Context) is
   begin
      Initialize_Binder_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Binder)));
   end Initialize_Binder;

   procedure Set_Binder (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Binder));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Binder_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Binder)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Binder)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Binder))) = Data);
   end Set_Binder;

   procedure Generic_Set_Binder (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Binder));
   begin
      if Length > 0 then
         Process_Binder (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder, Length));
      Initialize_Binder_Private (Ctx, Length);
   end Generic_Set_Binder;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Binder_Len := Get_Binder_Len (Ctx);
      Struct.Binder := (others => 0);
      Get_Binder (Ctx, Struct.Binder (Struct.Binder'First .. Struct.Binder'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Binder)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Binder_Len (Ctx, Struct.Binder_Len);
      Set_Binder (Ctx, Struct.Binder (Struct.Binder'First .. Struct.Binder'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Binder_Len) * 8) + 1) - 2));
   end To_Context;
