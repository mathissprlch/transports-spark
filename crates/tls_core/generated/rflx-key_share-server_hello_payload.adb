...
package body RFLX.Key_Share.Server_Hello_Payload
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Group =>
             Invalid (Ctx.Cursors (F_Key_Exchange_Len)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Key_Share.Server_Hello_Payload.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Key_Share.Server_Hello_Payload.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.Key_Share.Server_Hello_Payload.Has_Buffer (Ctx)
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Next (Ctx, Fld)
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Value (Fld, Val)
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Key_Share.Server_Hello_Payload.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Key_Share.Server_Hello_Payload.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Next (Ctx, Fld)
       and then Fld in F_Group | F_Key_Exchange_Len
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Value (Fld, Val)
       and then RFLX.Key_Share.Server_Hello_Payload.Valid_Size (Ctx, Fld, RFLX.Key_Share.Server_Hello_Payload.Field_Size (Ctx, Fld))
       and then RFLX.Key_Share.Server_Hello_Payload.Available_Space (Ctx, Fld) >= RFLX.Key_Share.Server_Hello_Payload.Field_Size (Ctx, Fld)
       and then RFLX.Key_Share.Server_Hello_Payload.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Key_Share.Server_Hello_Payload.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Group (Ctx : in out Context; Val : RFLX.TLS_Extensions.Named_Group) is
   begin
      Set_Scalar (Ctx, F_Group, RFLX.TLS_Extensions.To_Base_Integer (Val));
   end Set_Group;

   procedure Set_Key_Exchange_Len (Ctx : in out Context; Val : RFLX.Key_Share.Key_Exchange_Length) is
   begin
      Set_Scalar (Ctx, F_Key_Exchange_Len, RFLX.Key_Share.To_Base_Integer (Val));
   end Set_Key_Exchange_Len;
   ...
   begin
      ...
      pragma Assert (RFLX.Key_Share.Server_Hello_Payload.Valid_Length (Ctx, RFLX.Key_Share.Server_Hello_Payload.F_Key_Exchange, Length));
   ...
   begin
      ...
      Struct.Key_Exchange_Len := Get_Key_Exchange_Len (Ctx);
   ...
   begin
      ...
      Set_Key_Exchange_Len (Ctx, Struct.Key_Exchange_Len);
      Set_Key_Exchange (Ctx, Struct.Key_Exchange (Struct.Key_Exchange'First .. Struct.Key_Exchange'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Key_Exchange_Len) * 8) + 1) - 2));
   end To_Context;
