...
package RFLX.Record_Layer.Plaintext
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Type_Field, F_Legacy_Version, F_Length, F_Fragment, F_Final);

   subtype Field is Virtual_Field range F_Type_Field .. F_Fragment;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Well_Formed_Message (Ctx)
       and then RFLX.Record_Layer.Plaintext.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then Offset < RFLX.Record_Layer.Plaintext.Buffer_Length (Ctx)
       and then Pre (RFLX.Record_Layer.Plaintext.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Record_Layer.Plaintext.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, Fld)
       and then RFLX.Record_Layer.Plaintext.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Fragment =>
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, Fld);
   ...
   function Get_Type_Field (Ctx : Context) return RFLX.Record_Layer.Content_Type
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Valid (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field);

   function Get_Legacy_Version (Ctx : Context) return RFLX.Record_Layer.Protocol_Version
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Valid (Ctx, RFLX.Record_Layer.Plaintext.F_Legacy_Version);

   function Get_Length (Ctx : Context) return RFLX.Record_Layer.Length_U16
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Valid (Ctx, RFLX.Record_Layer.Plaintext.F_Length);
   ...
   with
     ...
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Well_Formed (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment),
     Post =>
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Well_Formed (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)),
     Post =>
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and RFLX.Record_Layer.Plaintext.Present (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment);
   ...
   procedure Set_Type_Field (Ctx : in out Context; Val : RFLX.Record_Layer.Content_Type)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field)
       and then RFLX.Record_Layer.Valid_Content_Type (RFLX.Record_Layer.To_Base_Integer (Val))
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field)
       and then RFLX.Record_Layer.Plaintext.Field_Condition (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Type_Field)
       and Get_Type_Field (Ctx) = Val
       and Invalid (Ctx, F_Legacy_Version)
       and Invalid (Ctx, F_Length)
       and Invalid (Ctx, F_Fragment)
       and Valid_Next (Ctx, F_Legacy_Version)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Type_Field) = Valid_Next (Ctx, F_Type_Field)'Old
       and Field_First (Ctx, F_Type_Field) = Field_First (Ctx, F_Type_Field)'Old;

   procedure Set_Legacy_Version (Ctx : in out Context; Val : RFLX.Record_Layer.Protocol_Version)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Legacy_Version)
       and then RFLX.Record_Layer.Valid_Protocol_Version (RFLX.Record_Layer.To_Base_Integer (Val))
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Legacy_Version) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Legacy_Version)
       and then RFLX.Record_Layer.Plaintext.Field_Condition (Ctx, RFLX.Record_Layer.Plaintext.F_Legacy_Version),
     Post =>
       Has_Buffer (Ctx)
       ...
       and Get_Type_Field (Ctx) = Get_Type_Field (Ctx)'Old
       and Field_First (Ctx, F_Legacy_Version) = Field_First (Ctx, F_Legacy_Version)'Old
       and (for all F in Field range F_Type_Field .. F_Type_Field =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));

   procedure Set_Length (Ctx : in out Context; Val : RFLX.Record_Layer.Length_U16)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Length)
       and then RFLX.Record_Layer.Valid_Length_U16 (RFLX.Record_Layer.To_Base_Integer (Val))
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Length) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Length)
       and then RFLX.Record_Layer.Plaintext.Field_Condition (Ctx, RFLX.Record_Layer.Plaintext.F_Length),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_Fragment)
       and Valid_Next (Ctx, F_Fragment)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and (for all F in Field range F_Type_Field .. F_Legacy_Version =>
               Context_Cursors_Index (Context_Cursors (Ctx), F) = Context_Cursors_Index (Context_Cursors (Ctx)'Old, F));
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Field_Condition (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment) = 0,
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     ...
     Post =>
       ...
       and then Get_Type_Field (Ctx) = Get_Type_Field (Ctx)'Old
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Valid_Length (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment, Data'Length)
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.Record_Layer.Plaintext.Field_Condition (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Fragment)
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)
       and then RFLX.Record_Layer.Plaintext.Valid_Length (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment, Length)
       and then RFLX_Types.To_Length (RFLX.Record_Layer.Plaintext.Available_Space (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment)) >= Length
   ...
   type Structure is
      record
         Type_Field : RFLX.Record_Layer.Content_Type;
         Legacy_Version : RFLX.Record_Layer.Protocol_Version;
         Length : RFLX.Record_Layer.Length_U16;
         Fragment : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 16639);
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Structure (Struct)
       and then RFLX.Record_Layer.Plaintext.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Type_Field (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Legacy_Version (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
   ...
   function Field_Size_Fragment (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Type_Field)) then True)
      and then (if Well_Formed (Cursors (F_Legacy_Version)) then Valid (Cursors (F_Type_Field)))
      and then (if Well_Formed (Cursors (F_Length)) then Valid (Cursors (F_Legacy_Version)))
      and then (if Well_Formed (Cursors (F_Fragment)) then Valid (Cursors (F_Length))))
   with
     Pre =>
   ...
   function Field_Size_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Length'Base is
     (case Fld is
          when F_Type_Field =>
             8,
          when F_Legacy_Version | F_Length =>
             16,
          when F_Fragment =>
   ...
   function Field_First_Type_Field (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Type_Field);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Type_Field =>
             Field_First_Type_Field (Cursors, First, Verified_Last, Written_Last),
          when F_Legacy_Version =>
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Type_Field))
                 then
                    (Cursors (F_Type_Field).Last - Cursors (F_Type_Field).First + 1 = 8
                     and then Cursors (F_Type_Field).First = First))
                and then (if
                             Well_Formed (Cursors (F_Legacy_Version))
                          then
                             (Cursors (F_Legacy_Version).Last - Cursors (F_Legacy_Version).First + 1 = 16
                              and then Cursors (F_Legacy_Version).First = Cursors (F_Type_Field).Last + 1))
                and then (if
                             Well_Formed (Cursors (F_Length))
                          then
                             (Cursors (F_Length).Last - Cursors (F_Length).First + 1 = 16
                              and then Cursors (F_Length).First = Cursors (F_Legacy_Version).Last + 1))
                and then (if
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Type_Field)
      and then RFLX.Record_Layer.Plaintext.Field_First (Ctx, RFLX.Record_Layer.Plaintext.F_Type_Field) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Type_Field) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Type_Field =>
             RFLX.Record_Layer.Valid_Content_Type (Val),
          when F_Legacy_Version =>
             RFLX.Record_Layer.Valid_Protocol_Version (Val),
          when F_Length =>
             RFLX.Record_Layer.Valid_Length_U16 (Val),
          when F_Fragment =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Type_Field | F_Legacy_Version | F_Length | F_Fragment =>
             True);
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Well_Formed (Ctx, F_Fragment));

   function Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Fragment));

   function Incomplete_Message (Ctx : Context) return Boolean is
     ((for some F in Field =>
          Incomplete (Ctx, F)));

   function Get_Type_Field (Ctx : Context) return RFLX.Record_Layer.Content_Type is
     (To_Actual (Ctx.Cursors (F_Type_Field).Value));

   function Get_Legacy_Version (Ctx : Context) return RFLX.Record_Layer.Protocol_Version is
     (To_Actual (Ctx.Cursors (F_Legacy_Version).Value));

   function Get_Length (Ctx : Context) return RFLX.Record_Layer.Length_U16 is
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Length) * 8 + 40);

   function Field_Size_Type_Field (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Legacy_Version (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Length (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Fragment (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Length) * 8);

end RFLX.Record_Layer.Plaintext;
