...
package RFLX.Pre_Shared_Key.Psk_Binder_Entry
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Binder_Len, F_Binder, F_Final);

   subtype Field is Virtual_Field range F_Binder_Len .. F_Binder;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Well_Formed_Message (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then Offset < RFLX.Pre_Shared_Key.Psk_Binder_Entry.Buffer_Length (Ctx)
       and then Pre (RFLX.Pre_Shared_Key.Psk_Binder_Entry.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Pre_Shared_Key.Psk_Binder_Entry.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Binder =>
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, Fld);
   ...
   function Get_Binder_Len (Ctx : Context) return RFLX.Pre_Shared_Key.Binder_Length
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len);

   pragma Warnings (On, "precondition is always False");

   function Get_Binder (Ctx : Context) return RFLX_Types.Bytes
   with
     Ghost,
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Well_Formed (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder),
     Post =>
       Get_Binder'Result'Length = RFLX_Types.To_Length (Field_Size (Ctx, F_Binder));

   procedure Get_Binder (Ctx : Context; Data : out RFLX_Types.Bytes)
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Well_Formed (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)),
     Post =>
       Equal (Ctx, F_Binder, Data);

   generic
      with procedure Process_Binder (Binder : RFLX_Types.Bytes);
   procedure Generic_Get_Binder (Ctx : Context)
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and RFLX.Pre_Shared_Key.Psk_Binder_Entry.Present (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder);
   ...
   procedure Set_Binder_Len (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Binder_Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len)
       and then RFLX.Pre_Shared_Key.Valid_Binder_Length (RFLX.Pre_Shared_Key.To_Base_Integer (Val))
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len) >= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Condition (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Binder_Len)
       and Get_Binder_Len (Ctx) = Val
       ...
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Binder_Len) = Valid_Next (Ctx, F_Binder_Len)'Old
       and Field_First (Ctx, F_Binder_Len) = Field_First (Ctx, F_Binder_Len)'Old;

   pragma Warnings (On, "aspect ""*"" not enforced on inlined subprogram ""*""");

   procedure Initialize_Binder (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder) >= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Binder)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Binder))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Binder) = Valid_Next (Ctx, F_Binder)'Old
       and then Get_Binder_Len (Ctx) = Get_Binder_Len (Ctx)'Old
       and then Field_First (Ctx, F_Binder) = Field_First (Ctx, F_Binder)'Old;

   procedure Set_Binder (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder) >= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder, Data'Length)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Condition (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Binder)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Binder))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Binder) = Valid_Next (Ctx, F_Binder)'Old
       and Get_Binder_Len (Ctx) = Get_Binder_Len (Ctx)'Old
       and Field_First (Ctx, F_Binder) = Field_First (Ctx, F_Binder)'Old
       and Equal (Ctx, F_Binder, Data);

   generic
      with procedure Process_Binder (Binder : out RFLX_Types.Bytes);
      with function Process_Data_Pre (Length : RFLX_Types.Length) return Boolean;
   procedure Generic_Set_Binder (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Next (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder) >= RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_Size (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder, Length)
       and then RFLX_Types.To_Length (RFLX.Pre_Shared_Key.Psk_Binder_Entry.Available_Space (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder)) >= Length
   ...
   type Structure is
      record
         Binder_Len : RFLX.Pre_Shared_Key.Binder_Length;
         Binder : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 254);
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Valid_Structure (Struct)
       and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Binder_Len (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Binder (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Binder_Len)) then True)
      and then (if Well_Formed (Cursors (F_Binder)) then Valid (Cursors (F_Binder_Len))))
   with
     Pre =>
   ...
   function Field_Size_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Length'Base is
     (case Fld is
          when F_Binder_Len =>
             8,
          when F_Binder =>
             RFLX_Types.Bit_Length (Cursors (F_Binder_Len).Value) * 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Field_First_Binder_Len (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Binder_Len);

   function Field_First_Binder (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Binder);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Binder_Len =>
             Field_First_Binder_Len (Cursors, First, Verified_Last, Written_Last),
          when F_Binder =>
             Field_First_Binder (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Binder_Len))
                 then
                    (Cursors (F_Binder_Len).Last - Cursors (F_Binder_Len).First + 1 = 8
                     and then Cursors (F_Binder_Len).First = First))
                and then (if
                             Well_Formed (Cursors (F_Binder))
                          then
                             (Cursors (F_Binder).Last - Cursors (F_Binder).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Binder_Len).Value) * 8
                              and then Cursors (F_Binder).First = Cursors (F_Binder_Len).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Binder_Len)
      and then RFLX.Pre_Shared_Key.Psk_Binder_Entry.Field_First (Ctx, RFLX.Pre_Shared_Key.Psk_Binder_Entry.F_Binder_Len) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Binder_Len) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Binder_Len =>
             RFLX.Pre_Shared_Key.Valid_Binder_Length (Val),
          when F_Binder =>
             True);

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Binder_Len | F_Binder =>
             True);
   ...
   function Get_Binder_Len (Ctx : Context) return RFLX.Pre_Shared_Key.Binder_Length is
     (To_Actual (Ctx.Cursors (F_Binder_Len).Value));
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.Base_Integer (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.Base_Integer (Struct.Binder_Len) * 8 + 8);

   function Field_Size_Binder_Len (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Binder (Struct : Structure) return RFLX_Types.Bit_Length is
     (RFLX_Types.Bit_Length (Struct.Binder_Len) * 8);

end RFLX.Pre_Shared_Key.Psk_Binder_Entry;
