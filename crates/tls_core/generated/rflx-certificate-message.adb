...
package body RFLX.Certificate.Message
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Context_Len =>
             Invalid (Ctx.Cursors (F_Context)),
          when F_Context =>
             Invalid (Ctx.Cursors (F_Cert_List_Len)),
          when F_Cert_List_Len =>
             Invalid (Ctx.Cursors (F_Cert_List)),
          when F_Cert_List =>
             True);
   ...
   with
     Pre =>
       ...
       and then RFLX.Certificate.Message.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Certificate.Message.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Cert_List then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Context (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Context;

   procedure Get_Context (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).Last);
   begin
   ...
   procedure Generic_Get_Context (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Context).Last);
   begin
      Process_Context (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Context;
   ...
   with
     Pre =>
       RFLX.Certificate.Message.Has_Buffer (Ctx)
       and then RFLX.Certificate.Message.Valid_Next (Ctx, Fld)
       and then RFLX.Certificate.Message.Valid_Value (Fld, Val)
       and then RFLX.Certificate.Message.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Certificate.Message.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Certificate.Message.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Certificate.Message.Valid_Next (Ctx, Fld)
       and then Fld in F_Context_Len | F_Cert_List_Len
       and then RFLX.Certificate.Message.Valid_Value (Fld, Val)
       and then RFLX.Certificate.Message.Valid_Size (Ctx, Fld, RFLX.Certificate.Message.Field_Size (Ctx, Fld))
       and then RFLX.Certificate.Message.Available_Space (Ctx, Fld) >= RFLX.Certificate.Message.Field_Size (Ctx, Fld)
       and then RFLX.Certificate.Message.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Certificate.Message.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Context_Len (Ctx : in out Context; Val : RFLX.Certificate.Context_Length) is
   begin
      Set_Scalar (Ctx, F_Context_Len, RFLX.Certificate.To_Base_Integer (Val));
   end Set_Context_Len;

   procedure Set_Cert_List_Len (Ctx : in out Context; Val : RFLX.Certificate.List_Length) is
   begin
      Set_Scalar (Ctx, F_Cert_List_Len, RFLX.Certificate.To_Base_Integer (Val));
   end Set_Cert_List_Len;

   procedure Set_Context_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Context, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Context_Empty;

   procedure Set_Cert_List_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Cert_List, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Cert_List_Empty;

   procedure Set_Cert_List (Ctx : in out Context; Seq_Ctx : RFLX.Certificate.Cert_Entry_List.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.Certificate.Cert_Entry_List.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Cert_List, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.Certificate.Cert_Entry_List.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Cert_List;

   procedure Initialize_Context_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Certificate.Message.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Context) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Context)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Context);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Context) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Context);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Context) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Context_Private;

   procedure Initialize_Context (Ctx : in out Context) is
   begin
      Initialize_Context_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Context)));
   end Initialize_Context;

   procedure Initialize_Cert_List_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Certificate.Message.Has_Buffer (Ctx)
       and then RFLX.Certificate.Message.Valid_Next (Ctx, RFLX.Certificate.Message.F_Cert_List)
       and then RFLX.Certificate.Message.Valid_Length (Ctx, RFLX.Certificate.Message.F_Cert_List, Length)
       and then RFLX_Types.To_Length (RFLX.Certificate.Message.Available_Space (Ctx, RFLX.Certificate.Message.F_Cert_List)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Cert_List)
       and then Field_Size (Ctx, F_Cert_List) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Cert_List)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cert_List);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cert_List) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Cert_List);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Cert_List) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Cert_List_Private;

   procedure Initialize_Cert_List (Ctx : in out Context) is
   begin
      Initialize_Cert_List_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Cert_List)));
   end Initialize_Cert_List;

   procedure Set_Context (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Context));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Context_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Context)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Context)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Context))) = Data);
   end Set_Context;

   procedure Generic_Set_Context (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Context));
   begin
      if Length > 0 then
         Process_Context (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Certificate.Message.Valid_Length (Ctx, RFLX.Certificate.Message.F_Context, Length));
      Initialize_Context_Private (Ctx, Length);
   end Generic_Set_Context;

   procedure Switch_To_Cert_List (Ctx : in out Context; Seq_Ctx : out RFLX.Certificate.Cert_Entry_List.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Cert_List);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Cert_List);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Cert_List) then
         Reset_Dependent_Fields (Ctx, F_Cert_List);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.Certificate.Cert_Entry_List.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Cert_List;

   procedure Update_Cert_List (Ctx : in out Context; Seq_Ctx : in out RFLX.Certificate.Cert_Entry_List.Context) is
      Valid_Sequence : constant Boolean := RFLX.Certificate.Message.Complete_Cert_List (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.Certificate.Cert_Entry_List.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Cert_List) := (State => S_Valid, First => Ctx.Cursors (F_Cert_List).First, Last => Ctx.Cursors (F_Cert_List).Last, Value => Ctx.Cursors (F_Cert_List).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Cert_List);
         Ctx.Cursors (F_Cert_List) := (others => <>);
      end if;
   end Update_Cert_List;
