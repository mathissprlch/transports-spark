...
package body RFLX.Pre_Shared_Key.Psk_Identity
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Pre_Shared_Key.Psk_Identity.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.Pre_Shared_Key.Psk_Identity.Has_Buffer (Ctx)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Value (Fld, Val)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Pre_Shared_Key.Psk_Identity.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Next (Ctx, Fld)
       and then Fld in F_Identity_Len | F_Obfuscated_Ticket_Age
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Value (Fld, Val)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Valid_Size (Ctx, Fld, RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, Fld))
       and then RFLX.Pre_Shared_Key.Psk_Identity.Available_Space (Ctx, Fld) >= RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, Fld)
       and then RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Pre_Shared_Key.Psk_Identity.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Identity_Len (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Identity_Length) is
   begin
      Set_Scalar (Ctx, F_Identity_Len, RFLX.Pre_Shared_Key.To_Base_Integer (Val));
   end Set_Identity_Len;

   procedure Set_Obfuscated_Ticket_Age (Ctx : in out Context; Val : RFLX.Pre_Shared_Key.Ticket_Age) is
   begin
      Set_Scalar (Ctx, F_Obfuscated_Ticket_Age, RFLX.Pre_Shared_Key.To_Base_Integer (Val));
   ...
   begin
      ...
      pragma Assert (RFLX.Pre_Shared_Key.Psk_Identity.Valid_Length (Ctx, RFLX.Pre_Shared_Key.Psk_Identity.F_Identity, Length));
      Initialize_Identity_Private (Ctx, Length);
   end Generic_Set_Identity;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Identity_Len := Get_Identity_Len (Ctx);
   ...
   begin
      Reset (Ctx);
      Set_Identity_Len (Ctx, Struct.Identity_Len);
      Set_Identity (Ctx, Struct.Identity (Struct.Identity'First .. Struct.Identity'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Identity_Len) * 8) + 1) - 2));
