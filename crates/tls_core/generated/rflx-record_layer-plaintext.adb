...
package body RFLX.Record_Layer.Plaintext
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Type_Field =>
             Invalid (Ctx.Cursors (F_Legacy_Version)),
          when F_Legacy_Version =>
             Invalid (Ctx.Cursors (F_Length)),
          when F_Length =>
             Invalid (Ctx.Cursors (F_Fragment)),
          when F_Fragment =>
             True);
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Record_Layer.Plaintext.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Fragment then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.Record_Layer.Plaintext.Has_Buffer (Ctx)
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, Fld)
       and then RFLX.Record_Layer.Plaintext.Valid_Value (Fld, Val)
       and then RFLX.Record_Layer.Plaintext.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Record_Layer.Plaintext.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Record_Layer.Plaintext.Composite_Field (Fld)
                 then
   ...
   with
     Pre =>
       ...
       and then RFLX.Record_Layer.Plaintext.Valid_Next (Ctx, Fld)
       and then Fld in F_Type_Field | F_Legacy_Version | F_Length
       and then RFLX.Record_Layer.Plaintext.Valid_Value (Fld, Val)
       and then RFLX.Record_Layer.Plaintext.Valid_Size (Ctx, Fld, RFLX.Record_Layer.Plaintext.Field_Size (Ctx, Fld))
       and then RFLX.Record_Layer.Plaintext.Available_Space (Ctx, Fld) >= RFLX.Record_Layer.Plaintext.Field_Size (Ctx, Fld)
       and then RFLX.Record_Layer.Plaintext.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Record_Layer.Plaintext.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Type_Field (Ctx : in out Context; Val : RFLX.Record_Layer.Content_Type) is
   begin
      Set_Scalar (Ctx, F_Type_Field, RFLX.Record_Layer.To_Base_Integer (Val));
   end Set_Type_Field;

   procedure Set_Legacy_Version (Ctx : in out Context; Val : RFLX.Record_Layer.Protocol_Version) is
   begin
      Set_Scalar (Ctx, F_Legacy_Version, RFLX.Record_Layer.To_Base_Integer (Val));
   end Set_Legacy_Version;

   procedure Set_Length (Ctx : in out Context; Val : RFLX.Record_Layer.Length_U16) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.Record_Layer.To_Base_Integer (Val));
   end Set_Length;
   ...
   begin
      ...
      pragma Assert (RFLX.Record_Layer.Plaintext.Valid_Length (Ctx, RFLX.Record_Layer.Plaintext.F_Fragment, Length));
      Initialize_Fragment_Private (Ctx, Length);
   end Generic_Set_Fragment;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Type_Field := Get_Type_Field (Ctx);
      Struct.Legacy_Version := Get_Legacy_Version (Ctx);
      Struct.Length := Get_Length (Ctx);
      Struct.Fragment := (others => 0);
      Get_Fragment (Ctx, Struct.Fragment (Struct.Fragment'First .. Struct.Fragment'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Fragment)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Type_Field (Ctx, Struct.Type_Field);
      Set_Legacy_Version (Ctx, Struct.Legacy_Version);
      Set_Length (Ctx, Struct.Length);
      Set_Fragment (Ctx, Struct.Fragment (Struct.Fragment'First .. Struct.Fragment'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Length) * 8) + 1) - 2));
   end To_Context;
