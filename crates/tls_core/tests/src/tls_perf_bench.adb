...
procedure Tls_Perf_Bench is
   ...
   procedure Report
     (Label : String; N : Positive; Bytes_Per_Op : Natural;
      Started_At : Time);
   procedure Report
     (Label : String; N : Positive; Bytes_Per_Op : Natural;
      Started_At : Time)
   is
      ...
      Mb_Per_Sec  : constant Float :=
        Float (N) * Float (Bytes_Per_Op)
        / 1_048_576.0
        / Float'Max (Float (Elapsed), 1.0e-9);
   begin
      Put_Line
        (Label & ":  " & Natural'Image (N) & " iters in"
         & Duration'Image (Elapsed) & "s = "
         & Integer'Image (Integer (Ops_Per_Sec)) & " op/s, "
         & Integer'Image (Integer (Mb_Per_Sec)) & " MiB/s");
   end Report;
   ...
   begin
      ...
      for Iter in 1 .. N loop
         ...
         begin
            ...
            declare
               Sf : constant Tls_Core.Octet_Array := Reply (1 .. Reply_Last);
            ...
            begin
               ...
               declare
                  Cf : constant Tls_Core.Octet_Array := Cf_Buf (1 .. Cf_Last);
