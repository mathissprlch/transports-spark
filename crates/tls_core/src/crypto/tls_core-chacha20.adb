...
is
   ...
   function Build_Initial_State
     (Key     : Key_Array;
      Nonce   : Nonce_Array;
      Counter : Word) return State_Array
   is
     ((0  => 16#6170_7865#,
       1  => 16#3320_646E#,
       2  => 16#7962_2D32#,
   ...
   function Run_Rounds_Fn (S0 : State_Array) return State_Array
   is
      S : State_Array := S0;
   begin
      for R in 1 .. 10 loop
   ...
   is
     (Spec_Rounds (Build_Initial_State (Key, Nonce, Counter)) (I)
   ...
   begin
      for I in 0 .. 15 loop
         ...
         begin
            ...
            Result (4 * I + 2) := Octet (Shift_Right (W,  8)
                                           and 16#FF#);
            Result (4 * I + 3) := Octet (Shift_Right (W, 16)
                                           and 16#FF#);
            Result (4 * I + 4) := Octet (Shift_Right (W, 24)
                                           and 16#FF#);
         end;
         ...
         begin
            ...
            pragma Assert
              (Spec_State_Word (Key, Nonce, Counter, I)
                 = Spec_Rounds (Build_Initial_State (Key, Nonce, Counter))
                   ...
                   + Build_Initial_State (Key, Nonce, Counter) (I));
            pragma Assert
              (Build_Initial_State (Key, Nonce, Counter) = Initial);
            pragma Assert
              (Spec_Rounds (Build_Initial_State (Key, Nonce, Counter))
                 = Spec_Rounds (Initial));
            ...
            pragma Loop_Invariant
              (for all K in 1 .. 4 * (I + 1) =>
                 Out_Block (K)
                   = Spec_Block_Bytes (Key, Nonce, Counter) (K));
         end;
      end loop;
   ...
   begin
      ...
      if Input'Length = 0 then
         ...
         pragma Assert
           (Output = Spec_Chacha20 (Key, Nonce, Counter, Input));
         return;
      end if;
      ...
      while Cursor < Input'Length loop
         ...
         declare
            ...
            Old_Cursor : constant Natural := Cursor with Ghost;
            Old_Output : constant Octet_Array := Output with Ghost;
         begin
