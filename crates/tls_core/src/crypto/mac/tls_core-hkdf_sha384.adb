...
is
   ...
   begin
      ...
      Tls_Core.Hmac_Sha384.Compute
        (Key => PRK, Message => Buf, Out_Tag => Result);
      return Result;
      ...
      while Cursor < L loop
         ...
         begin
            for K in 1 .. Take loop
               ...
               pragma
                 Loop_Invariant
                   (for all J in 1 .. K => OKM (Cursor + J) = T_Curr (J));
            end loop;
            Cursor := Cursor + Take;
         end;
   ...
   procedure Hmac_Expand
     (Prk : Octet_Array; Info : Octet_Array; Output : out Octet_Array) is
   begin
