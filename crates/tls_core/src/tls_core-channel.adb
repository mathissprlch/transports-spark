...
is
   ...
   is
      Encrypted_Len : constant Natural := Plaintext'Length + 16;
      ...
      Ct : Octet_Array (1 .. Plaintext'Length);
      Tag : Tag_Bytes;
   begin
      Out_Buf := (others => 0);
      Aead.Seal_Record
        (S          => D.Stream,
         ...
         Tag        => Tag);
      Out_Buf (1 .. 5) := AAD;
      Out_Buf (6 .. 5 + Plaintext'Length) := Ct;
      Out_Buf
        (5 + Plaintext'Length + 1 .. 5 + Plaintext'Length + 16) := Tag;
      Out_Last := 5 + Plaintext'Length + 16;
   end Send;
   ...
   begin
      ...
      begin
         ...
         Pt_Len := Frag_Len - 16;
         if Pt_Len <= 0 or else Out_Buf'Length < Pt_Len then
            return;
         end if;
         AAD := (Op_Type, Ver_Hi, Ver_Lo,
                 In_Buf (F + 3), In_Buf (F + 4));
         declare
            Ct  : constant Octet_Array := In_Buf (F + 5 .. F + 4 + Pt_Len);
            Tag : Tag_Bytes;
            Pt  : Octet_Array (1 .. Pt_Len);
            OK_Local : Boolean;
         begin
            Tag := In_Buf
              (F + 5 + Pt_Len .. F + 4 + Pt_Len + 16);
            Aead.Open_Record
              (S          => D.Stream,
               Key        => D.Key,
               AAD        => AAD,
