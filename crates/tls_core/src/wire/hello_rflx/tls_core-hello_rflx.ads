...
is
   ...
   function Spec_Sid_Len (B : Octet_Array) return Natural
   is (Natural (B (35)))
   with
     Ghost,
     Pre => B'First = 1 and then B'Length >= 40 and then B (35) <= 32;

   function Spec_Suite_Offset (B : Octet_Array) return Positive
   is (36 + Spec_Sid_Len (B))
   with
     Ghost,
     Pre =>
       B'First = 1
       and then B'Length >= 40
       and then B (35) <= 32
       and then 36 + Natural (B (35)) + 1 <= B'Last;

   function Spec_Suite_Code (B : Octet_Array) return Tls_Core.Suites.U16
   is (Tls_Core.Suites.U16 (B (Spec_Suite_Offset (B)))
       * 256
   ...
   function Spec_Random (B : Octet_Array) return Random_Bytes
   is (B (3 .. 34))
   with Ghost, Pre => B'First = 1 and then B'Length >= 40;

   function Spec_Valid (B : Octet_Array) return Boolean
   is (B (1) = 16#03#
       and then B (2) = 16#03#
       and then B (35) <= 32
   ...
   with
     Pre  => Out_Buf'First = 1 and then Out_Buf'Length >= 44,
     Post =>
       Out_Last = 43
