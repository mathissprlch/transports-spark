...
procedure Parse (Der : Octet_Array; P : out Parsed_Cert; OK : out Boolean) is
...
begin
   ...
   Read_Tlv (Der, Pos, T, VP, VL, Next_Pos, Step_OK);
   if not Step_OK or else T /= Tag_Integer or else Next_Pos > P.Tbs_Last + 1
   then
      return;
   end if;
   Pos := Next_Pos;
   ...
   Read_Tlv (Der, Pos, T, VP, VL, Next_Pos, Step_OK);
   if not Step_OK or else T /= Tag_Sequence or else Next_Pos > P.Tbs_Last + 1
   then
      return;
   end if;
   Pos := Next_Pos;
