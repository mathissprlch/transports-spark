...
begin
   ...
   Read_TLV_Header
     (Buf, Outer_BF, Tag_Sequence, Alg_BF, Alg_BL, Alg_AF, Step_OK);
   if not Step_OK then
      return;
   end if;
   ...
   --  algorithm OID
   Read_TLV_Header (Buf, Alg_BF, Tag_Oid, Oid_BF, Oid_BL, Oid_AF, Step_OK);
   if not Step_OK then
      return;
   end if;
   ...
   if Oid_BF <= Oid_BL and then Slice_Equal (Buf, Oid_BF, Oid_BL, Oid_Rsa) then
      Kind := Rsa;
   ...
   then
      ...
      begin
         ...
         Read_TLV_Header
           (Buf, Params_Cur, Tag_Oid, Curve_BF, Curve_BL, Curve_AF, Sub_OK);
         if not Sub_OK then
            return;
         end if;
         ...
         if Curve_BF <= Curve_BL
           and then Slice_Equal (Buf, Curve_BF, Curve_BL, Oid_P256)
         then
            Kind := Ecdsa_P256;
         else
            return;
   ...
   Read_TLV_Header (Buf, Alg_AF, Tag_Bit_String, Bs_BF, Bs_BL, Bs_AF, Step_OK);
   if not Step_OK then
      OK := False;
      Kind := Unknown;
      return;
