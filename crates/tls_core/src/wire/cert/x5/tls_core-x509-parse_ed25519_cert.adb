...
begin
   ...
   Read_Tlv
     (Der, Tbs_Tlv_Pos, Tbs_Tag, Tbs_V_Pos, Tbs_V_Len, Tbs_Next, Tbs_OK);
   ...
   if V_Len /= 1 + 64 or else Der (V_Pos) /= 0 then
      return;
   end if;
   for I in 1 .. 64 loop
      Sig (I) := Der (V_Pos + I);
   ...
   declare
      ...
      Bs_OK : Boolean;
   begin
      Read_Tlv (Der, Pos, Spki_Tag, Spki_VP, Spki_VL, Spki_Nx, Spki_OK);
      ...
      Inner_Pos := Spki_VP;
      Read_Tlv (Der, Inner_Pos, Alg_T, Alg_VP, Alg_VL, Alg_Nx, Alg_OK);
      ...
      Read_Tlv (Der, Alg_Nx, Bs_T, Bs_VP, Bs_VL, Bs_Nx, Bs_OK);
      if not Bs_OK or else Bs_T /= Tag_Bit_String then
         return;
      end if;
      if Bs_VL /= 1 + 32 or else Der (Bs_VP) /= 0 then
         return;
      end if;
