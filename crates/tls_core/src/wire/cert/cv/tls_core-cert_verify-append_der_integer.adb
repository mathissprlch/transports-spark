...
procedure Append_Der_Integer
  (Value : Octet_Array; Out_Buf : in out Octet_Array; Cursor : in out Natural)
is
   ...
   Body_Len      : Natural;
begin
   while First_Nonzero <= 32 and then Value (First_Nonzero) = 0 loop
   ...
   Body_Len := (32 - First_Nonzero + 1) + (if Need_Pad then 1 else 0);
