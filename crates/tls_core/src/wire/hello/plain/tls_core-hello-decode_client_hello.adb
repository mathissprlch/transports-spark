...
begin
   ...
   if CH.Session_Id_Len > 32 then
      return;
   end if;
   if CH.Session_Id_Len > 0 then
      if P + CH.Session_Id_Len - 1 > In_Bytes'Last then
         return;
      end if;
   ...
   if not Read_OK or else U16_Val < 2 or else U16_Val mod 2 /= 0 then
      return;
   end if;
   ...
   if Body_L - Body_F + 1 < 2 + 4 + 32 then
      return;
   end if;
