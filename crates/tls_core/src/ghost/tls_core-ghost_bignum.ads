...
is
   ...
   Max_Limbs : constant :=
     160;          --  >= an RSA-2048 product in 26-bit limbs
   ...
   with Pre => In_Bounds (A, Add_Cap), Post => A + Zero = A;

   procedure Lemma_Add_Assoc (A, B, C : Big_Nat)
   with
     Pre  =>
       In_Bounds (A, Assoc_Cap)
       and then In_Bounds (B, Assoc_Cap)
   ...
   is (if T = 0
       ...
       else Mul_Col (A, B, K, T - 1) + Mul_Limb (A (T)) * Mul_Limb (B (K - T)))
   with
     Pre                =>
   ...
   with
     Pre                =>
       In_Bounds (W, Mul_Cap)
       and then In_Bounds (Wc, Mul_Cap)
       and then In_Bounds (P, Mul_Cap)
       and then In_Bounds (R, Mul_Cap)
       and then Kc in 0 .. 5
       and then T <= K,
     Post               =>
       Diff3_Col'Result
       in -(LLI (T + 1) * (7 * Two_Pow_54)) .. LLI (T + 1) * (7 * Two_Pow_54),
     Subprogram_Variant => (Decreases => T);
   ...
   with
     Pre                =>
       ...
       and then T <= K,
     Post               =>
       Mul_Col (W, R, K, T) - Mul_Col (Wc, R, K, T) - Kc * Mul_Col (P, R, K, T)
   ...
   procedure Lemma_Mul_Col_Zero (A, B : Big_Nat; K, T, Na, Nb : Limb_Index)
   with
     Pre                =>
       In_Bounds (A, Mul_Cap)
       and then In_Bounds (B, Mul_Cap)
   ...
   with
     Pre  =>
       ...
       and then AB = A * B,
     Post =>
       (for all K in Limb_Index => (if K >= Na + Nb - 1 then AB (K) = 0));
   ...
   with
     Pre  => X >= 0,
     Post =>
       Lo26 (X) in 0 .. In_Cap and then X = Lo26 (X) + Limb_Base * Hi26 (X);
   ...
   with Pre => X in 0 .. Prod_Cap, Post => Hi26 (X) in 0 .. Hi_Cap;
   ...
   with Pre => In_Bounds (A, Add_Cap), Post => Val_Eq (A, A, Zero_Carry);
   ...
   with
     Pre  =>
       ...
       and then SC_Bounded (C1)
       and then SC_Bounded (C2)
       and then SC_Bounded (Add_Carry (C1, C2))
       and then SVal_Eq (A, B, C1)
       and then SVal_Eq (B, D, C2),
   ...
   with
     Pre  =>
       In_Bounds (A, Rechain_Cap)
       and then In_Bounds (B, Rechain_Cap)
       and then SC_Wide (C)
   ...
   function Conv_Chain_Col
     (C : Carry_Array; R : Big_Nat; K, T : Limb_Index) return LLI
   is (if T = 0
       then C (0) * R (K)
   ...
   with
     Pre  =>
       In_Bounds (W, Mul_Cap)
       and then In_Bounds (Wc, In_Cap)
       and then In_Bounds (P, In_Cap)
   ...
   is ([for J in 0 .. Max_Limbs => (if J = I + 1 then Hi26 (A (I)) else 0)])
   with
     Pre  => In_Bounds (A, Prod_Cap) and then I < Max_Limbs - 1,
   ...
   with
     ...
     Post =>
       (for all I in Limb_Index => Smul'Result (I) = K * A (I))
   ...
   with
     ...
     Post =>
       In_Bounds (Sweep5_Out'Result, Add_Cap)
   ...
   with Pre => In_Bounds (B, Mul_Cap), Post => Sweep5_Out (B) (5) <= 2;
   ...
   function Sw9_C0 (A : Big_Nat) return LLI
   is (Hi26 (A (0)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C0'Result in 0 .. Hi_Cap;

   function Sw9_C1 (A : Big_Nat) return LLI
   is (Hi26 (A (1) + Sw9_C0 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C1'Result in 0 .. Hi_Cap;

   function Sw9_C2 (A : Big_Nat) return LLI
   is (Hi26 (A (2) + Sw9_C1 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C2'Result in 0 .. Hi_Cap;

   function Sw9_C3 (A : Big_Nat) return LLI
   is (Hi26 (A (3) + Sw9_C2 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C3'Result in 0 .. Hi_Cap;

   function Sw9_C4 (A : Big_Nat) return LLI
   is (Hi26 (A (4) + Sw9_C3 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C4'Result in 0 .. Hi_Cap;

   function Sw9_C5 (A : Big_Nat) return LLI
   is (Hi26 (A (5) + Sw9_C4 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C5'Result in 0 .. Hi_Cap;

   function Sw9_C6 (A : Big_Nat) return LLI
   is (Hi26 (A (6) + Sw9_C5 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C6'Result in 0 .. Hi_Cap;

   function Sw9_C7 (A : Big_Nat) return LLI
   is (Hi26 (A (7) + Sw9_C6 (A)))
   with Pre => In_Bounds (A, Prod_Cap), Post => Sw9_C7'Result in 0 .. Hi_Cap;

   function Sw9_C8 (A : Big_Nat) return LLI
   is (Hi26 (A (8) + Sw9_C7 (A)))
   ...
   with
     Pre  => In_Bounds (A, Prod_Cap),
     Post =>
       In_Bounds (Sweep9_Out'Result, Add_Cap)
   ...
   with
     Pre  =>
       (for all I in Limb_Index range 0 .. 4 => B (I) in 0 .. In_Cap)
       and then B (5) in 0 .. Fold_C_Cap,
     Post =>
       In_Bounds (Fold_Out'Result, Add_Cap)
   ...
   with
     Pre  =>
       ...
       and then B (5) in 0 .. Fold_C_Cap,
     Post =>
       In_Bounds (Fold_Plus_P'Result, Add_Cap)
       and then (for all I in Limb_Index range 0 .. 4 =>
   ...
   with
     Pre  => C4 in 0 .. Fold_C_Cap,
     Post => Carry_Bounded (Fold_Chain'Result);
   ...
   is (B (4) = In_Cap
       ...
       and then B (2) = In_Cap
       and then B (1) = In_Cap
       and then B (0) >= In_Cap - 4);

   function Subtract_P5_Out (B : Big_Nat) return Big_Nat
   is (if Sub_Cond (B) then [0 => B (0) - (In_Cap - 4), others => 0] else B)
   with
     Pre  =>
       (for all I in Limb_Index range 0 .. 4 => B (I) in 0 .. In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
   ...
   with
     Pre  =>
       X (0) in 0 .. 2**27 - 1
       and then (for all I in Limb_Index range 1 .. 4 => X (I) in 0 .. In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   X (I) = 0),
     Post =>
       (if Sweep5_Out (X) (5) >= 1
        then (for all I in Limb_Index range 1 .. 4 => Sweep5_Out (X) (I) = 0));
   ...
   with
     Pre  =>
       In_Bounds (B, Prod_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 => B (I) = 0)
       and then Sweep5_Out (B) (5) = 0,
     Post =>
       In_Bounds (Reduce_Canonical'Result, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
   ...
   with
     Pre  =>
       K1 in 0 .. Smul_Cap
       and then K2 in 0 .. Smul_Cap
       and then K1 + K2 <= Smul_Cap
       and then In_Bounds (A, In_Cap),
   ...
   with
     Pre  =>
       ...
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 => B (I) = 0)
       and then not Sub_Cond (A)
       and then not Sub_Cond (B)
       and then Ka in 0 .. Mult_Cap
       and then Kb in 0 .. Mult_Cap
       and then SC_Bounded (C)
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   B (I) = 0),
     Post =>
       In_Bounds (Normalize'Result.Val, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
       ...
       and then Normalize'Result.PMult = Smul (Normalize'Result.KMult, P_Prime)
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   B (I) = 0),
     Post =>
       In_Bounds (Canonical'Result, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
   ...
   with
     ...
     Pre  =>
       In_Bounds (A, In_Cap)
       and then In_Bounds (N, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 => A (I) = 0)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   N (I) = 0),
     Post =>
       In_Bounds (Field_Add'Result, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
   ...
   with
     ...
     Pre  =>
       ...
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   R (I) = 0),
     Post =>
       In_Bounds (Field_Mul'Result, In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
       ...
       and then Field_Mul'Result
                = Canonical
                    (Carry_Model (Fold_High_9_Out (Sweep9_Out (A * R))));

   --  Zero is its own canonical form.
   procedure Lemma_Canonical_Zero
   with Ghost, Post => Canonical (Zero) = Zero;
   ...
   with Ghost, Post => Canonical (P_Prime) = Zero;
   ...
   with
     Pre  =>
       (for all I in Limb_Index range 0 .. 4 => R (I) in 0 .. In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   R (I) = 0),
     Post =>
       (for all I in Limb_Index range 0 .. 4 =>
          Shift1'Result (I) in 0 .. In_Cap)
   ...
   with
     Pre  =>
       ...
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
                   R (I) = 0),
     Post =>
       (for all I in Limb_Index range 0 .. 8 =>
          Shift2'Result (I) in 0 .. In_Cap)
       and then (for all I in Limb_Index range 9 .. Max_Limbs - 1 =>
                   Shift2'Result (I) = 0);
   ...
   with
     ...
     Post =>
       Val_Eq
         (Fold_High_Plus_P (Shift2 (R)),
          Shift2 (R),
          Fold_High_Chain (Shift2 (R)));
   ...
   with
     ...
     Post =>
       Val_Eq
         (Fold_High_Plus_P (Shift3 (R)),
          Shift3 (R),
          Fold_High_Chain (Shift3 (R)));
   ...
   with
     ...
     Post =>
       Val_Eq
         (Fold_High_Plus_P (Shift4 (R)),
          Shift4 (R),
          Fold_High_Chain (Shift4 (R)));
   ...
   is ([0 => B (5), 1 => B (6), 2 => B (7), 3 => B (8), others => 0])
   with
     Pre  => (for all I in Limb_Index range 5 .. 8 => B (I) in 0 .. In_Cap),
     Post =>
       In_Bounds (High4'Result, In_Cap)
   ...
   with
     ...
     Post =>
       In_Bounds (Fold_High_9_Out'Result, Round1_Out_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 =>
   ...
   with
     ...
     Post =>
       SVal_Eq
         (R1, Fold_Plus_P (T), Add_Carry (D1, Neg_Carry (Fold_Chain (T (5)))));
   ...
   with
     Pre  =>
       ...
       and then B (9) in 0 .. Fold9_Top_Cap,
     Post =>
       In_Bounds (Fold_High_9_PrimePart'Result, Add_Cap)
       and then (for all I in Limb_Index range 0 .. 8 =>
   ...
   procedure Lemma_Mul_Reduce (Conv, S, R1, T : Big_Nat; C1, D1 : Carry_Array)
   with
     Pre  =>
       In_Bounds (Conv, Conv_Col_Cap)
       and then (for all I in Limb_Index range 9 .. Max_Limbs - 1 =>
       ...
       and then S = Sweep9_Out (Conv)
       and then C1 = Sweep9_Chain (Conv)
       and then (for all I in Limb_Index range 0 .. 8 => S (I) in 0 .. In_Cap)
       and then S (9) in 0 .. Fold9_Top_Cap
       ...
       and then T = Sweep5_Out (R1)
       and then D1 = Sweep5_Chain (R1)
       and then (for all I in Limb_Index range 0 .. 4 => T (I) in 0 .. In_Cap)
       and then T (5) in 0 .. Fold_C_Cap,
     Post =>
       SVal_Eq
         (Conv,
   ...
   with
     Pre  =>
       In_Bounds (B, Carry_In_Cap)
       and then (for all I in Limb_Index range 5 .. Max_Limbs - 1 => B (I) = 0)
       and then Sweep5_Out (B) (5) in 0 .. Fold_C_Cap,
     Post =>
       In_Bounds (Carry_Model'Result, Add_Cap)
     ...
     Post =>
       SVal_Eq
         (B,
          ...
          Add_Carry
            (Sweep5_Chain (B), Neg_Carry (Fold_Chain (Sweep5_Out (B) (5)))));
