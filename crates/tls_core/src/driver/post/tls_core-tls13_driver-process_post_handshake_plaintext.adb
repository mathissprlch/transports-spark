...
begin
   ...
   if Plaintext (Plaintext'First) = Hs_Type_New_Session_Ticket then
      ...
      declare
         Hi  : constant Natural := Natural (Plaintext (Plaintext'First + 1));
         Mid : constant Natural := Natural (Plaintext (Plaintext'First + 2));
         Lo  : constant Natural := Natural (Plaintext (Plaintext'First + 3));
      ...
      begin
         ...
         declare
            Body_Slice : constant Octet_Array (1 .. L) :=
              Plaintext (Plaintext'First + 4 .. Plaintext'First + 3 + L);
   ...
   elsif Plaintext (Plaintext'First) = Tls_Core.Key_Update.Hs_Type_Key_Update
   then
