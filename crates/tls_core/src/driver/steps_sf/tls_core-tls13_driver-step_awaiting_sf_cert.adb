            ...
            if Cursor <= In_Bytes'Last then
               Logger.Log
                 (Logger.Debug,
                  ...
                  & " cursor="
                  & Natural'Image (Cursor)
                  & " last="
                  & Natural'Image (In_Bytes'Last));
            end if;
                  ...
                  case Sub is
                     ...
                     when Expect_Cert       =>
                        ...
                        begin
                           ...
                           Tls_Core.Cert_Verify.Decode_Body_Single
                             (Body_Bytes, OK, Cert_F, Cert_L);
                           if not OK
                             or else Cert_L < Cert_F
                             or else Cert_L - Cert_F + 1 > Leaf_Buf'Length
                           then
                              --  Use Decode_Error to
                              --  distinguish from
                     ...
                     when Expect_CertVerify =>
                        if Msg_Last < 4 + 4
                          or else Msg_Buf (1) /= Hs_Type_Cert_Verify
                        then
                           Fail_Encrypted
                             (D,
                              Tls_Core.Alert.Desc_Decode_Error,
                        ...
                        begin
                           ...
                           Tls_Core.Cert_Verify.Decode_Body
                             (Body_Bytes, OK, Sig_Scheme, Sig_F, Sig_L);
                           Logger.Log
                             (Logger.Debug,
                           ...
                           if not OK or else Sig_Scheme /= 16#0403# then
                              Fail_Encrypted
                                (D,
                                 Tls_Core.Alert.Desc_Decode_Error,
                                 Out_Buf,
                           ...
                           declare
                              ...
                              Result    :
                                Tls_Core.Cert_Chain.Validation_Result;
                           begin
                              if Total_Len < 16 then
                                 Fail_Encrypted
                                   (D,
                                    Tls_Core.Alert.Desc_Bad_Certificate,
                                    Out_Buf,
                                    Out_Last);
                                 return;
                              end if;
                              All_Certs (1 .. Leaf_Len) :=
                                Leaf_Buf (1 .. Leaf_Len);
                              All_Certs (Leaf_Len + 1 .. Total_Len) :=
                                D.Trust_Anchor_Bytes (1 .. D.Trust_Anchor_Len);
                              Chain_In.Count := 1;
                              Chain_In.Entries (1) :=
                                (First => 1, Last => Leaf_Len);
                              Trust.Count := D.Trust_Anchor_Spec.Count;
                              for I in 1 .. D.Trust_Anchor_Spec.Count loop
                                 pragma
                                   Loop_Invariant
                                     (I in 1 .. D.Trust_Anchor_Spec.Count);
                                 Trust.Entries (I) :=
                                   (First =>
                                      D.Trust_Anchor_Spec.Entries (I).First
                                      + Leaf_Len,
                                    Last  =>
                                      D.Trust_Anchor_Spec.Entries (I).Last
                                      + Leaf_Len);
                              end loop;
                              Logger.Log
                                (Logger.Debug,
                              ...
                              Tls_Core.Cert_Chain.Authenticate_Server
                                (All_Certs       => All_Certs,
                                 ...
                                 Sig_Body        =>
                                   Body_Bytes (Sig_F .. Sig_L),
                                 Transcript_Hash =>
                                   Th_After_Cert
                                     (1
                                      .. Tls_Core.Key_Sched.Hash_Len
                                           (D.Suite)),
                                 Result          => Result);
                              Logger.Log
                                (Logger.Debug,
                                 "sf_cert: cv auth result="
                                 & Tls_Core.Cert_Chain.Validation_Result'Image
                                     (Result));
                              if not Tls_Core.Cert_Chain."="
                                       (Result,
                                        Tls_Core.Cert_Chain.OK_Validated)
                              then
                                 Fail_Encrypted
                                   (D,
                                    Tls_Core.Alert.Desc_Bad_Certificate,
                     ...
                     when Expect_SF         =>
                        ...
                        loop
                           pragma Loop_Invariant (I in 1 .. 48);
                           Diff :=
