...
is
   ...
   with Pre => Out_Buf'First = 1 and then Out_Buf'Length >= 7;
   ...
   procedure Fail_Encrypted
     (D           : in out Driver;
      ...
      Out_Buf     : out Octet_Array;
      Out_Last    : out Natural)
   with Pre => Out_Buf'First = 1 and then Out_Buf'Length >= 5 + 2 + 1 + 16;
   ...
   with
     ...
     Post => Out_Last = 4 + Body_Bytes'Length;
   ...
   with
     ...
     Post => Out_Last = 5 + Hs_Bytes'Length;
