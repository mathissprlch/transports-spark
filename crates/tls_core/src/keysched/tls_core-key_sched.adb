...
is
      ...
      if Suite = Tls_Core.Suites.Aes_256_Gcm_Sha384 then
         ...
         begin
            ...
            Tls_Core.Key_Schedule_Sha384.Derive_Secret
              (Secret_In  => E,
               ...
               Out_Secret => D1);
            Tls_Core.Key_Schedule_Sha384.Extract
              (Salt => D1, IKM => Ecdhe_Shared, Out_PRK => H);
            Exp384
              (Secret  => H,
               Label   => C_Hs_Lab,
               Context => Th_After_Sh (1 .. HH),
               Output  => C);
            Exp384
              (Secret  => H,
               Label   => S_Hs_Lab,
         ...
         begin
            Tls_Core.Key_Schedule.Extract
              (Salt    => Z32,
               IKM     => PSK (PSK'First .. PSK'First + 31),
               Out_PRK => E);
   ...
   begin
      ...
      if Suite = Tls_Core.Suites.Aes_256_Gcm_Sha384 then
         ...
         begin
            ...
            Exp384
              (Secret  => Hs_Secret (1 .. HH),
               Label   => Derived_Lab,
               Context => EH,
               Output  => D2);
            Tls_Core.Key_Schedule_Sha384.Extract
              (Salt => D2, IKM => Z48, Out_PRK => M);
            Exp384
              (Secret  => M,
               Label   => C_Ap_Lab,
               Context => Th_After_Sf (1 .. HH),
               Output  => C);
            Exp384
              (Secret  => M,
               Label   => S_Ap_Lab,
         ...
         begin
            ...
            Exp256
              (Secret  => Hs_Secret (1 .. HL),
               Label   => Derived_Lab,
               Context => EH,
               Output  => D2);
            Tls_Core.Key_Schedule.Extract
              (Salt => D2, IKM => Z32, Out_PRK => M);
            Exp256
              (Secret  => M,
               Label   => C_Ap_Lab,
               Context => Th_After_Sf (1 .. HL),
               Output  => C);
            Exp256
              (Secret  => M,
               Label   => S_Ap_Lab,
   ...
   begin
      ...
      if Suite = Tls_Core.Suites.Aes_256_Gcm_Sha384 then
         ...
         begin
            Exp384
              (Secret  => Base_Key (1 .. HH),
               Label   => Finished_Lab,
               Context => Empty,
               Output  => FK);
            Tls_Core.Hmac_Sha384.Compute
              (Key => FK, Message => Transcript_Hash (1 .. HH), Out_Tag => R);
            Out_Verify (1 .. HH) := R;
         end;
      else
         declare
         ...
         begin
            Exp256
              (Secret  => Base_Key (1 .. HL),
               Label   => Finished_Lab,
               Context => Empty,
               Output  => FK);
            Tls_Core.Hmac_Sha256.Compute
              (Key => FK, Message => Transcript_Hash (1 .. HL), Out_Tag => R);
            Out_Verify (1 .. HL) := R;
         end;
      end if;
   end Build_Finished;
   ...
   begin
      ...
      if Suite = Tls_Core.Suites.Aes_256_Gcm_Sha384 then
         ...
         begin
            Exp384
              (Secret  => Master_Secret (1 .. HH),
               Label   => Res_Master_Lab,
               Context => Th_After_Cf (1 .. HH),
               Output  => R);
