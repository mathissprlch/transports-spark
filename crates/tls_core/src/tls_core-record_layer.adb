...
is
   ...
   with
     ...
     Post =>
       From_Bytes
         (Seq_Byte (X, 1), Seq_Byte (X, 2),
          Seq_Byte (X, 3), Seq_Byte (X, 4),
          Seq_Byte (X, 5), Seq_Byte (X, 6),
          Seq_Byte (X, 7), Seq_Byte (X, 8))
       = X;
   ...
   with
     Ghost,
     Pre  => A /= B,
     Post => (for some I in 1 .. 8 =>
                Seq_Byte (A, I) /= Seq_Byte (B, I));
