...
procedure Grpc_Core_Tests is
   ...
   begin
      ...
      Check ("length BE bytes",
             Frame (2) = 0 and Frame (3) = 0
             and Frame (4) = 0 and Frame (5) = 11);
   ...
   begin
      ...
      for Code in Grpc_Core.Status.Code'Range loop
         ...
         Grpc_Core.Status.From_String
           (Buf (Buf'First .. Last), C, OK);
         if not OK or C /= Code then
            All_Round_Trip := False;
            Put_Line ("  mismatch " & Code'Image
                      & " → " & Buf (Buf'First .. Last)
                      & " → " & C'Image);
         end if;
      end loop;
