...
package body Tls_Transport is
   ...
   Ccs_Type : constant Tls_Core.Octet :=
     Tls_Core.Octet
       (RFLX.Record_Layer.To_Base_Integer
   ...
   begin
      ...
      declare
         Rec_Len : constant Natural :=
           Natural (Buf (Buf'First + 3)) * 256 + Natural (Buf (Buf'First + 4));
      begin
         if Rec_Len > 16640 or else Buf'First + 5 + Rec_Len - 1 > Buf'Last then
            return;
         end if;
         ...
         if Rflx_Ptr /= null then
            ...
            begin
               ...
               begin
                  ...
                  if not RFLX.Record_Layer.Plaintext.Well_Formed_Message (Ctx)
                  then
   ...
   procedure Handshake_Loop (Chan : in out Channel) is
      use Tls_Core;
   ...
   begin
      loop
         exit when
           Tls13_Driver.Current_State (Chan.Driver) = Done
         ...
         loop
            if Current_State (Chan.Driver) = Awaiting_SF then
               ...
               Read_One_Record
                 (Chan.Tcp, In_Buf, In_Last, In_OK, Chan.Rflx_Buf);
            end if;
            if not In_OK then
               Logger.Log (Logger.Error, "hs: EOF during handshake");
               raise Connect_Error with "TLS: EOF during handshake";
            end if;
            exit when In_Last < 1 or else In_Buf (In_Buf'First) /= Ccs_Type;
      ...
      if Tls13_Driver.Current_State (Chan.Driver) /= Done then
         Logger.Log
           (Logger.Error,
            "hs: handshake failed at state="
            & State'Image (Current_State (Chan.Driver)));
         raise Connect_Error with "TLS: handshake failed";
      end if;

      Open_App_Directions
        (Chan.Driver, Out_Dir => Chan.App_Out, In_Dir => Chan.App_In);
      Chan.Open := True;
   end Handshake_Loop;
   ...
   begin
      ...
      begin
         for I in Host_Bytes'Range loop
            Host_Bytes (I) := Octet (Character'Pos (Config.Hostname (I)));
         end loop;
         Trust_Spec.Count := 1;
         Trust_Spec.Entries (1) := (First => 1, Last => Config.Trust_Der_Len);
         Tls13_Driver.Init_Cert_Client
           (D                  => Chan.Driver,
            Trust_Anchor_Bytes => Config.Trust_Der (1 .. Config.Trust_Der_Len),
         ...
         if Config.Alpn_Len > 0 then
            ...
            begin
               for I in Alpn_Bytes'Range loop
                  Alpn_Bytes (I) := Octet (Character'Pos (Config.Alpn (I)));
               end loop;
      ...
      Tls13_Driver.Step
        (Chan.Driver, Tls_Core.Octet_Array'(1 .. 0 => 0), Out_Buf, Out_Last);
      if Out_Last > 0 then
         Tcp_Transport.Send_All (Chan.Tcp, Out_Buf (1 .. Out_Last));
      end if;
   ...
   begin
      ...
      Ready := Status = Completed and then Is_Set (Read_Set, Sock);
   exception
      when others =>
         Ready := False;
   end Poll_Internal;
   ...
   procedure Send (Chan : in out Channel; Data : Tls_Core.Octet_Array) is
      use Tls_Core;
   ...
   procedure Refill_Pending (Chan : in out Channel; OK : out Boolean) is
      use Tls_Core;
   ...
   begin
      ...
      begin
         Chain_Spec.Count := 1;
         Chain_Spec.Entries (1) := (First => 1, Last => Config.Cert_Der_Len);
         Tls13_Driver.Init_Cert_Server
           (D                => Chan.Driver,
            Cert_Chain_Bytes => Config.Cert_Der (1 .. Config.Cert_Der_Len),
            Chain_Spec       => Chain_Spec,
            Sign_Priv_Key    => Config.Key_Raw (1 .. 32),
            Sig_Alg          => Tls_Core.Suites.Sig_Ecdsa_Secp256r1_Sha256,
            Ecdhe_Priv       => Ecdhe_Seed);
         if Config.Hostname_Len > 0 then
            declare
               Host_Bytes : Octet_Array (1 .. Config.Hostname_Len);
            begin
               for I in Host_Bytes'Range loop
                  Host_Bytes (I) :=
                    Octet (Character'Pos (Config.Hostname (I)));
               end loop;
               Tls13_Driver.Set_Sni_Hostname (Chan.Driver, Host_Bytes);
            end;
         end if;
            ...
            begin
               ...
               then
                  ...
                  begin
                     Name :=
                       Alpn_Bytes
                         (Alpn_Bytes'First + 1 .. Alpn_Bytes'First + N_Len);
