...
is
   ...
   procedure Initialize_Opaque_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Opaque_Data) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Opaque_Data)
   ...
   begin
      ...
      Ctx.Cursors (F_Opaque_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   ...
   procedure Generic_Set_Opaque_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Opaque_Data));
   begin
      if Length > 0 then
         Process_Opaque_Data (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Ping.Packet.Valid_Length (Ctx, RFLX.Ping.Packet.F_Opaque_Data, Length));
   ...
   begin
      Struct.Opaque_Data := (others => 0);
      Get_Opaque_Data (Ctx, Struct.Opaque_Data (Struct.Opaque_Data'First .. Struct.Opaque_Data'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Opaque_Data)) + 1) - 2));
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Opaque_Data (Ctx, Struct.Opaque_Data);
   end To_Context;
