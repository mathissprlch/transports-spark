...
package body RFLX.Settings.Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Settings.Packet.Valid_Value (Fld, Val)
       and then RFLX.Settings.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Settings.Packet.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Settings.Packet.Composite_Field (Fld)
                 then
   ...
   procedure Set_Parameters_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Parameters, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Parameters_Empty;

   procedure Set_Parameters (Ctx : in out Context; Seq_Ctx : RFLX.Settings.Parameter_List.Context) is
      Size : constant RFLX_Types.Bit_Length := RFLX_Types.To_Bit_Length (RFLX.Settings.Parameter_List.Byte_Size (Seq_Ctx));
      Unused_First, Unused_Last : RFLX_Types.Bit_Index;
      Buffer_First, Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Parameters, 0, Size, True, Buffer_First, Buffer_Last, Unused_Offset);
      RFLX.Settings.Parameter_List.Copy (Seq_Ctx, Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Parameters;

   procedure Initialize_Parameters_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       ...
       and then RFLX_Types.To_Length (RFLX.Settings.Packet.Available_Space (Ctx, RFLX.Settings.Packet.F_Parameters)) >= Length,
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Parameters)
       and then Field_Size (Ctx, F_Parameters) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Parameters)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Parameters);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Parameters) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Parameters);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Parameters) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Parameters_Private;

   procedure Initialize_Parameters (Ctx : in out Context; Length : RFLX_Types.Length) is
   begin
      Initialize_Parameters_Private (Ctx, Length);
   end Initialize_Parameters;

   procedure Switch_To_Parameters (Ctx : in out Context; Seq_Ctx : out RFLX.Settings.Parameter_List.Context) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Parameters);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_Parameters);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      if Invalid (Ctx, F_Parameters) then
         Reset_Dependent_Fields (Ctx, F_Parameters);
         pragma Warnings (Off, "attribute Update is an obsolescent feature");
      ...
      RFLX.Settings.Parameter_List.Initialize (Seq_Ctx, Buffer, First, Last);
      pragma Warnings (On, "unused assignment to ""Buffer""");
   end Switch_To_Parameters;

   procedure Update_Parameters (Ctx : in out Context; Seq_Ctx : in out RFLX.Settings.Parameter_List.Context) is
      Valid_Sequence : constant Boolean := RFLX.Settings.Packet.Complete_Parameters (Ctx, Seq_Ctx);
      Buffer : RFLX_Types.Bytes_Ptr;
   begin
      RFLX.Settings.Parameter_List.Take_Buffer (Seq_Ctx, Buffer);
      Ctx.Buffer := Buffer;
      if Valid_Sequence then
         Ctx.Cursors (F_Parameters) := (State => S_Valid, First => Ctx.Cursors (F_Parameters).First, Last => Ctx.Cursors (F_Parameters).Last, Value => Ctx.Cursors (F_Parameters).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Parameters);
         Ctx.Cursors (F_Parameters) := (others => <>);
      end if;
