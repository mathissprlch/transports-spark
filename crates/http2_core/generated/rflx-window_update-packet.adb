...
package body RFLX.Window_Update.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Reserved =>
             Invalid (Ctx.Cursors (F_Window_Size_Increment)),
   ...
   with
     Pre =>
       ...
       and RFLX.Window_Update.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Window_Update.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Window_Size_Increment then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Window_Update.Packet.Valid_Value (Fld, Val)
       and then RFLX.Window_Update.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Window_Update.Packet.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if
                              Fld in F_Window_Size_Increment
   ...
   with
     Pre =>
       ...
       and then RFLX.Window_Update.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Reserved | F_Window_Size_Increment
       and then RFLX.Window_Update.Packet.Valid_Value (Fld, Val)
       and then RFLX.Window_Update.Packet.Valid_Size (Ctx, Fld, RFLX.Window_Update.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Window_Update.Packet.Available_Space (Ctx, Fld) >= RFLX.Window_Update.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Window_Update.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Window_Update.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Window_Update.Reserved_Bit) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Window_Update.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Window_Size_Increment (Ctx : in out Context; Val : RFLX.Window_Update.Window_Size_Increment) is
   begin
      Set_Scalar (Ctx, F_Window_Size_Increment, RFLX.Window_Update.To_Base_Integer (Val));
   end Set_Window_Size_Increment;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Reserved := Get_Reserved (Ctx);
      Struct.Window_Size_Increment := Get_Window_Size_Increment (Ctx);
   end To_Structure;
   ...
   begin
      ...
      Set_Window_Size_Increment (Ctx, Struct.Window_Size_Increment);
   end To_Context;
