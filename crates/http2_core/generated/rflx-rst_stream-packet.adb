...
package body RFLX.Rst_Stream.Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Rst_Stream.Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Rst_Stream.Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Rst_Stream.Packet.Valid_Value (Fld, Val)
       and then RFLX.Rst_Stream.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Rst_Stream.Packet.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Rst_Stream.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Error_Code
       and then RFLX.Rst_Stream.Packet.Valid_Value (Fld, Val)
       and then RFLX.Rst_Stream.Packet.Valid_Size (Ctx, Fld, RFLX.Rst_Stream.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Rst_Stream.Packet.Available_Space (Ctx, Fld) >= RFLX.Rst_Stream.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Rst_Stream.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Rst_Stream.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Error_Code (Ctx : in out Context; Val : RFLX.Rst_Stream.Error_Code) is
   begin
      Set_Scalar (Ctx, F_Error_Code, RFLX.Rst_Stream.To_Base_Integer (Val));
   end Set_Error_Code;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Error_Code := Get_Error_Code (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Error_Code (Ctx, Struct.Error_Code);
   end To_Context;
