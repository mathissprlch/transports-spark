...
package body RFLX.Headers.Packet
with
  SPARK_Mode
is
   ...
   with
     ...
     Post =>
       ...
       and Invalid (Ctx, F_Field_Block_Fragment)
   is
   begin
   ...
   function Get_Field_Block_Fragment (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Field_Block_Fragment;

   procedure Get_Field_Block_Fragment (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).Last);
   begin
   ...
   procedure Generic_Get_Field_Block_Fragment (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Field_Block_Fragment).Last);
   begin
      Process_Field_Block_Fragment (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Field_Block_Fragment;
   ...
   with
     Pre =>
       ...
       and then RFLX.Headers.Packet.Valid_Value (Fld, Val)
       and then RFLX.Headers.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Headers.Packet.Available_Space (Ctx, Fld)
       and then (if RFLX.Headers.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   procedure Set_Field_Block_Fragment_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Field_Block_Fragment, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Field_Block_Fragment_Empty;

   procedure Initialize_Field_Block_Fragment_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
     ...
     Post =>
       ...
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Field_Block_Fragment)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Field_Block_Fragment);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Field_Block_Fragment) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Field_Block_Fragment);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Field_Block_Fragment) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Field_Block_Fragment_Private;

   procedure Initialize_Field_Block_Fragment (Ctx : in out Context; Length : RFLX_Types.Length) is
   begin
      Initialize_Field_Block_Fragment_Private (Ctx, Length);
   end Initialize_Field_Block_Fragment;

   procedure Set_Field_Block_Fragment (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Field_Block_Fragment));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Field_Block_Fragment_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Field_Block_Fragment)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Field_Block_Fragment)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Field_Block_Fragment))) = Data);
   end Set_Field_Block_Fragment;

   procedure Generic_Set_Field_Block_Fragment (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Field_Block_Fragment));
   begin
      if Length > 0 then
         Process_Field_Block_Fragment (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Headers.Packet.Valid_Length (Ctx, RFLX.Headers.Packet.F_Field_Block_Fragment, Length));
      Initialize_Field_Block_Fragment_Private (Ctx, Length);
   end Generic_Set_Field_Block_Fragment;
