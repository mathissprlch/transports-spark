     ...
     Post =>
       ...
       and then (if
                 ...
                 then
                    ...
                    and Stream.Open.FSM.Written_Last (Ctx, B_Inbound) = Stream.Open.FSM.Written_Last (Ctx, B_Inbound)'Old)
       and then (if
                    Ext_Buf /= B_Outgoing
                 then
                    Has_Buffer (Ctx, B_Outgoing) = Has_Buffer (Ctx, B_Outgoing)'Old
                    and Stream.Open.FSM.Written_Last (Ctx, B_Outgoing) = Stream.Open.FSM.Written_Last (Ctx, B_Outgoing)'Old)
...
private
   ...
   type Private_Context is
      record
         Next_State : State := S_Awaiting_Headers;
         Inbound_Ctx : Frame.Packet.Context;
         Outgoing_Ctx : Frame.Packet.Context;
      end record;

   function Uninitialized (Ctx : Context) return Boolean is
     (not Frame.Packet.Has_Buffer (Ctx.P.Inbound_Ctx)
      and not Frame.Packet.Has_Buffer (Ctx.P.Outgoing_Ctx));
   ...
   function Buffers_Initialized (Ctx : Context) return Boolean is
     (Frame.Packet.Has_Buffer (Ctx.P.Inbound_Ctx)
      and then Frame.Packet.Has_Buffer (Ctx.P.Outgoing_Ctx));
   ...
   function Channel_Accessible (St : State; Chan : Channel) return Boolean is
     (case Chan is
          when C_Network =>
             (case St is
                 when S_Awaiting_Headers | S_Awaiting_Body | S_Sending_Response =>
                    True,
                 when others =>
                    False),
          when C_App_Pending =>
