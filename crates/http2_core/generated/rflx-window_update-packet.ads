...
package RFLX.Window_Update.Packet
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Reserved, F_Window_Size_Increment, F_Final);

   subtype Field is Virtual_Field range F_Reserved .. F_Window_Size_Increment;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Window_Update.Packet.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Well_Formed_Message (Ctx)
       and then RFLX.Window_Update.Packet.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then Offset < RFLX.Window_Update.Packet.Buffer_Length (Ctx)
       and then Pre (RFLX.Window_Update.Packet.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Window_Update.Packet.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Window_Update.Packet.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Window_Update.Packet.Valid_Next (Ctx, Fld);
   ...
   function Get_Reserved (Ctx : Context) return RFLX.Window_Update.Reserved_Bit
   with
     Pre =>
       RFLX.Window_Update.Packet.Valid (Ctx, RFLX.Window_Update.Packet.F_Reserved);

   function Get_Window_Size_Increment (Ctx : Context) return RFLX.Window_Update.Window_Size_Increment
   with
     Pre =>
       RFLX.Window_Update.Packet.Valid (Ctx, RFLX.Window_Update.Packet.F_Window_Size_Increment);
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Window_Update.Reserved_Bit)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Valid_Next (Ctx, RFLX.Window_Update.Packet.F_Reserved)
       and then RFLX.Window_Update.Valid_Reserved_Bit (RFLX.Window_Update.To_Base_Integer (Val))
       and then RFLX.Window_Update.Packet.Available_Space (Ctx, RFLX.Window_Update.Packet.F_Reserved) >= RFLX.Window_Update.Packet.Field_Size (Ctx, RFLX.Window_Update.Packet.F_Reserved)
       and then RFLX.Window_Update.Packet.Field_Condition (Ctx, RFLX.Window_Update.Packet.F_Reserved),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Reserved)
       and Invalid (Ctx, F_Window_Size_Increment)
       and Valid_Next (Ctx, F_Window_Size_Increment)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_Window_Size_Increment (Ctx : in out Context; Val : RFLX.Window_Update.Window_Size_Increment)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Valid_Next (Ctx, RFLX.Window_Update.Packet.F_Window_Size_Increment)
       and then RFLX.Window_Update.Valid_Window_Size_Increment (RFLX.Window_Update.To_Base_Integer (Val))
       and then RFLX.Window_Update.Packet.Available_Space (Ctx, RFLX.Window_Update.Packet.F_Window_Size_Increment) >= RFLX.Window_Update.Packet.Field_Size (Ctx, RFLX.Window_Update.Packet.F_Window_Size_Increment)
       and then RFLX.Window_Update.Packet.Field_Condition (Ctx, RFLX.Window_Update.Packet.F_Window_Size_Increment),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Window_Size_Increment)
       and Get_Window_Size_Increment (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Window_Size_Increment))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Window_Size_Increment) = Valid_Next (Ctx, F_Window_Size_Increment)'Old
       and Field_First (Ctx, F_Window_Size_Increment) = Field_First (Ctx, F_Window_Size_Increment)'Old
   ...
   type Structure is
      record
         Reserved : RFLX.Window_Update.Reserved_Bit;
         Window_Size_Increment : RFLX.Window_Update.Window_Size_Increment;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Window_Update.Packet.Has_Buffer (Ctx)
       and then RFLX.Window_Update.Packet.Valid_Structure (Struct)
       and then RFLX.Window_Update.Packet.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Window_Size_Increment (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Reserved)) then True)
      and then (if Well_Formed (Cursors (F_Window_Size_Increment)) then Valid (Cursors (F_Reserved))))
   with
     Pre =>
   ...
   function Field_First_Window_Size_Increment (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 1)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Window_Size_Increment);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Window_Size_Increment =>
             Field_First_Window_Size_Increment (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Window_Size_Increment))
                          then
                             (Cursors (F_Window_Size_Increment).Last - Cursors (F_Window_Size_Increment).First + 1 = 31
                              and then Cursors (F_Window_Size_Increment).First = Cursors (F_Reserved).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Window_Update.Packet.Field_First (Ctx, RFLX.Window_Update.Packet.F_Reserved) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Reserved =>
             RFLX.Window_Update.Valid_Reserved_Bit (Val),
          when F_Window_Size_Increment =>
             RFLX.Window_Update.Valid_Window_Size_Increment (Val));

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Reserved | F_Window_Size_Increment =>
             True);
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Window_Size_Increment));
   ...
   function Get_Reserved (Ctx : Context) return RFLX.Window_Update.Reserved_Bit is
     (To_Actual (Ctx.Cursors (F_Reserved).Value));

   function Get_Window_Size_Increment (Ctx : Context) return RFLX.Window_Update.Window_Size_Increment is
     (To_Actual (Ctx.Cursors (F_Window_Size_Increment).Value));
   ...
   function Field_Size_Window_Size_Increment (Struct : Structure) return RFLX_Types.Bit_Length is
     (31);

end RFLX.Window_Update.Packet;
