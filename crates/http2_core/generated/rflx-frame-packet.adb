...
package body RFLX.Frame.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Length =>
             Invalid (Ctx.Cursors (F_Frame_Type)),
          when F_Frame_Type =>
             Invalid (Ctx.Cursors (F_Flags)),
          when F_Flags =>
             Invalid (Ctx.Cursors (F_Reserved)),
          when F_Reserved =>
             Invalid (Ctx.Cursors (F_Stream_Identifier)),
   ...
   with
     Pre =>
       ...
       and then RFLX.Frame.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Frame.Packet.Composite_Field (Fld)
   is
   ...
   with
     Pre =>
       RFLX.Frame.Packet.Has_Buffer (Ctx)
       and then RFLX.Frame.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Frame.Packet.Valid_Value (Fld, Val)
       and then RFLX.Frame.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Frame.Packet.Available_Space (Ctx, Fld)
       and then (if RFLX.Frame.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Frame.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Length | F_Frame_Type | F_Flags | F_Reserved | F_Stream_Identifier
       and then RFLX.Frame.Packet.Valid_Value (Fld, Val)
       and then RFLX.Frame.Packet.Valid_Size (Ctx, Fld, RFLX.Frame.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Frame.Packet.Available_Space (Ctx, Fld) >= RFLX.Frame.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Frame.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Frame.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.Frame.Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.Frame.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Frame_Type (Ctx : in out Context; Val : RFLX.Http2_Parameters.HTTP_2_Frame_Type_Enum) is
   begin
      Set_Scalar (Ctx, F_Frame_Type, RFLX.Http2_Parameters.To_Base_Integer (Val));
   end Set_Frame_Type;

   procedure Set_Flags (Ctx : in out Context; Val : RFLX.Frame.Flags) is
   begin
      Set_Scalar (Ctx, F_Flags, RFLX.Frame.To_Base_Integer (Val));
   end Set_Flags;

   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Frame.Reserved_Bit) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Frame.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Stream_Identifier (Ctx : in out Context; Val : RFLX.Frame.Stream_Identifier) is
   begin
      Set_Scalar (Ctx, F_Stream_Identifier, RFLX.Frame.To_Base_Integer (Val));
   end Set_Stream_Identifier;

   procedure Set_Frame_Type (Ctx : in out Context; Val : RFLX.Http2_Parameters.HTTP_2_Frame_Type)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Frame.Packet.Has_Buffer (Ctx)
       and then RFLX.Frame.Packet.Valid_Next (Ctx, RFLX.Frame.Packet.F_Frame_Type)
       and then RFLX.Http2_Parameters.Valid_HTTP_2_Frame_Type (Val)
   ...
   begin
      ...
      pragma Assert (RFLX.Frame.Packet.Valid_Length (Ctx, RFLX.Frame.Packet.F_Payload, Length));
   ...
   begin
      ...
      Struct.Frame_Type := Get_Frame_Type (Ctx);
      Struct.Flags := Get_Flags (Ctx);
      Struct.Reserved := Get_Reserved (Ctx);
      Struct.Stream_Identifier := Get_Stream_Identifier (Ctx);
   ...
   begin
      ...
      Set_Frame_Type (Ctx, Struct.Frame_Type);
      Set_Flags (Ctx, Struct.Flags);
      Set_Reserved (Ctx, Struct.Reserved);
      Set_Stream_Identifier (Ctx, Struct.Stream_Identifier);
