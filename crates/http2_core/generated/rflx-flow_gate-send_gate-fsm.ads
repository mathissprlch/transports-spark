...
with RFLX.Flow_Gate.Send_Gate.FSM_Allocator;
with RFLX.RFLX_Types;
with RFLX.Flow_Gate.Op_Packet;
with RFLX.Flow_Gate.Decision_Packet;

package RFLX.Flow_Gate.Send_Gate.FSM
with
  SPARK_Mode
is
   ...
   type Channel is (C_App_Outbox, C_App_Decision);

   type State is (S_Idle, S_Dispatch, S_Check_Send, S_Approve_Send, S_Deny_Send, S_Apply_Conn, S_Bump_Conn_OK, S_Apply_Stream, S_Bump_Stream_OK, S_Init_Stream_Window, S_Flow_Error, S_Emit_Decision, S_Final);
...
private
   ...
   type Private_Context is
      record
         Next_State : State := S_Idle;
         Conn_Window : Flow_Gate.Window_Bytes := 65535;
         Stream_Window : Flow_Gate.Window_Bytes := 65535;
         Pending_Ctx : Flow_Gate.Op_Packet.Context;
         Reply_Ctx : Flow_Gate.Decision_Packet.Context;
         Slots : Flow_Gate.Send_Gate.FSM_Allocator.Slots;
         Memory : Flow_Gate.Send_Gate.FSM_Allocator.Memory;
      end record;

   function Uninitialized (Ctx : Context) return Boolean is
     (not Flow_Gate.Op_Packet.Has_Buffer (Ctx.P.Pending_Ctx)
      and not Flow_Gate.Decision_Packet.Has_Buffer (Ctx.P.Reply_Ctx)
      and Flow_Gate.Send_Gate.FSM_Allocator.Uninitialized (Ctx.P.Slots));

   function Global_Initialized (Ctx : Context) return Boolean is
     (Flow_Gate.Op_Packet.Has_Buffer (Ctx.P.Pending_Ctx)
      and then Ctx.P.Pending_Ctx.Buffer_First = RFLX_Types.Index'First
      and then Ctx.P.Pending_Ctx.Buffer_Last = RFLX_Types.Index'First + 4095
      and then Flow_Gate.Decision_Packet.Has_Buffer (Ctx.P.Reply_Ctx)
      and then Ctx.P.Reply_Ctx.Buffer_First = RFLX_Types.Index'First
      and then Ctx.P.Reply_Ctx.Buffer_Last = RFLX_Types.Index'First + 4095);

   function Global_Allocated (Ctx : Context) return Boolean is
     (Flow_Gate.Send_Gate.FSM_Allocator.Global_Allocated (Ctx.P.Slots));
          ...
          when C_App_Decision =>
             (case Ctx.P.Next_State is
                 when S_Emit_Decision =>
                    Flow_Gate.Decision_Packet.Well_Formed_Message (Ctx.P.Reply_Ctx)
                    and Flow_Gate.Decision_Packet.Byte_Size (Ctx.P.Reply_Ctx) > 0,
                 when others =>
                    False));
   ...
   function Read_Buffer_Size (Ctx : Context; Chan : Channel) return RFLX_Types.Length is
     (case Chan is
          ...
          when C_App_Decision =>
             (case Ctx.P.Next_State is
                 when S_Emit_Decision =>
                    Flow_Gate.Decision_Packet.Byte_Size (Ctx.P.Reply_Ctx),
                 when others =>
                    RFLX_Types.Unreachable));
   ...
   function Write_Buffer_Size (Ctx : Context; Chan : Channel) return RFLX_Types.Length is
     (case Chan is
          when C_App_Outbox =>
             (case Ctx.P.Next_State is
                 when S_Idle =>
                    Flow_Gate.Op_Packet.Buffer_Length (Ctx.P.Pending_Ctx),
                 when others =>
                    RFLX_Types.Unreachable),
          when C_App_Decision =>
             0);
