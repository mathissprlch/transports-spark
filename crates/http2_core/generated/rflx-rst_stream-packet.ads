...
package RFLX.Rst_Stream.Packet
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Error_Code, F_Final);

   subtype Field is Virtual_Field range F_Error_Code .. F_Error_Code;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Rst_Stream.Packet.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Well_Formed_Message (Ctx)
       and then RFLX.Rst_Stream.Packet.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then Offset < RFLX.Rst_Stream.Packet.Buffer_Length (Ctx)
       and then Pre (RFLX.Rst_Stream.Packet.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Rst_Stream.Packet.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Rst_Stream.Packet.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Valid_Next (Ctx, Fld);
   ...
   function Get_Error_Code (Ctx : Context) return RFLX.Rst_Stream.Error_Code
   with
     Pre =>
       RFLX.Rst_Stream.Packet.Valid (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code);
   ...
   procedure Set_Error_Code (Ctx : in out Context; Val : RFLX.Rst_Stream.Error_Code)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Valid_Next (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code)
       and then RFLX.Rst_Stream.Valid_Error_Code (RFLX.Rst_Stream.To_Base_Integer (Val))
       and then RFLX.Rst_Stream.Packet.Available_Space (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code) >= RFLX.Rst_Stream.Packet.Field_Size (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code)
       and then RFLX.Rst_Stream.Packet.Field_Condition (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Error_Code)
       and Get_Error_Code (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Error_Code))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   type Structure is
      record
         Error_Code : RFLX.Rst_Stream.Error_Code;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Rst_Stream.Packet.Has_Buffer (Ctx)
       and then RFLX.Rst_Stream.Packet.Valid_Structure (Struct)
       and then RFLX.Rst_Stream.Packet.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Error_Code (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Error_Code)) then True))
   with
     Pre =>
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Rst_Stream.Packet.Field_First (Ctx, RFLX.Rst_Stream.Packet.F_Error_Code) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Error_Code) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Error_Code));
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Error_Code =>
             RFLX.Rst_Stream.Valid_Error_Code (Val));
   ...
   function Get_Error_Code (Ctx : Context) return RFLX.Rst_Stream.Error_Code is
   ...
   function Field_Size_Error_Code (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.Rst_Stream.Packet;
