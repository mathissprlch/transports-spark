...
package RFLX.Settings.Parameter
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Identifier, F_Value, F_Final);

   subtype Field is Virtual_Field range F_Identifier .. F_Value;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Settings.Parameter.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Well_Formed_Message (Ctx)
       and then RFLX.Settings.Parameter.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then Offset < RFLX.Settings.Parameter.Buffer_Length (Ctx)
       and then Pre (RFLX.Settings.Parameter.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Settings.Parameter.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, Fld)
       and then RFLX.Settings.Parameter.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Valid_Next (Ctx, Fld);
   ...
   function Get_Identifier (Ctx : Context) return RFLX.Http2_Parameters.HTTP_2_Settings
   with
     Pre =>
       RFLX.Settings.Parameter.Valid (Ctx, RFLX.Settings.Parameter.F_Identifier);

   function Get_Value (Ctx : Context) return RFLX.Settings.Setting_Value
   with
     Pre =>
       RFLX.Settings.Parameter.Valid (Ctx, RFLX.Settings.Parameter.F_Value);
   ...
   procedure Set_Identifier (Ctx : in out Context; Val : RFLX.Http2_Parameters.HTTP_2_Settings_Enum)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, RFLX.Settings.Parameter.F_Identifier)
       and then RFLX.Http2_Parameters.Valid_HTTP_2_Settings (RFLX.Http2_Parameters.To_Base_Integer (Val))
       and then RFLX.Settings.Parameter.Available_Space (Ctx, RFLX.Settings.Parameter.F_Identifier) >= RFLX.Settings.Parameter.Field_Size (Ctx, RFLX.Settings.Parameter.F_Identifier)
       and then RFLX.Settings.Parameter.Field_Condition (Ctx, RFLX.Settings.Parameter.F_Identifier),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Identifier)
       and Get_Identifier (Ctx) = (True, Val)
   ...
   procedure Set_Value (Ctx : in out Context; Val : RFLX.Settings.Setting_Value)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, RFLX.Settings.Parameter.F_Value)
       and then RFLX.Settings.Valid_Setting_Value (RFLX.Settings.To_Base_Integer (Val))
       and then RFLX.Settings.Parameter.Available_Space (Ctx, RFLX.Settings.Parameter.F_Value) >= RFLX.Settings.Parameter.Field_Size (Ctx, RFLX.Settings.Parameter.F_Value)
       and then RFLX.Settings.Parameter.Field_Condition (Ctx, RFLX.Settings.Parameter.F_Value),
     Post =>
       Has_Buffer (Ctx)
       ...
       and (for all F in Field range F_Identifier .. F_Identifier =>
   ...
   type Structure is
      record
         Identifier : RFLX.Http2_Parameters.HTTP_2_Settings;
         Value : RFLX.Settings.Setting_Value;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Structure (Struct)
       and then RFLX.Settings.Parameter.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Identifier (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Identifier)) then True)
      and then (if Well_Formed (Cursors (F_Value)) then Valid (Cursors (F_Identifier))))
   with
     Pre =>
      ...
      and then ((if
                 ...
                 then
                    (Cursors (F_Identifier).Last - Cursors (F_Identifier).First + 1 = 16
                     and then Cursors (F_Identifier).First = First))
                and then (if
                             Well_Formed (Cursors (F_Value))
                          then
                             (Cursors (F_Value).Last - Cursors (F_Value).First + 1 = 32
                              and then Cursors (F_Value).First = Cursors (F_Identifier).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Settings.Parameter.Field_First (Ctx, RFLX.Settings.Parameter.F_Identifier) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Identifier) = Ctx.Last - Ctx.First + 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Identifier =>
             RFLX.Http2_Parameters.Valid_HTTP_2_Settings (Val),
          when F_Value =>
             RFLX.Settings.Valid_Setting_Value (Val));

   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Identifier | F_Value =>
             True);
   ...
   function Get_Identifier (Ctx : Context) return RFLX.Http2_Parameters.HTTP_2_Settings is
     (To_Actual (Ctx.Cursors (F_Identifier).Value));

   function Get_Value (Ctx : Context) return RFLX.Settings.Setting_Value is
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (RFLX.Http2_Parameters.Valid_HTTP_2_Settings (Struct.Identifier));
   ...
   function Field_Size_Identifier (Struct : Structure) return RFLX_Types.Bit_Length is
     (16);

   function Field_Size_Value (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.Settings.Parameter;
