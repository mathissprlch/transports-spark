...
package RFLX.Flow_Gate.Decision_Packet
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Kind, F_Bytes, F_Final);

   subtype Field is Virtual_Field range F_Kind .. F_Bytes;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Well_Formed_Message (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then Offset < RFLX.Flow_Gate.Decision_Packet.Buffer_Length (Ctx)
       and then Pre (RFLX.Flow_Gate.Decision_Packet.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Flow_Gate.Decision_Packet.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Flow_Gate.Decision_Packet.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, Fld);
   ...
   function Get_Kind (Ctx : Context) return RFLX.Flow_Gate.Decision_Kind
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Valid (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind);

   function Get_Bytes (Ctx : Context) return RFLX.Flow_Gate.Window_Bytes
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Valid (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Bytes);
   ...
   procedure Set_Kind (Ctx : in out Context; Val : RFLX.Flow_Gate.Decision_Kind)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind)
       and then RFLX.Flow_Gate.Valid_Decision_Kind (RFLX.Flow_Gate.To_Base_Integer (Val))
       and then RFLX.Flow_Gate.Decision_Packet.Available_Space (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind) >= RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind)
       and then RFLX.Flow_Gate.Decision_Packet.Field_Condition (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Kind)
       and Get_Kind (Ctx) = Val
       and Invalid (Ctx, F_Bytes)
       and Valid_Next (Ctx, F_Bytes)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_Bytes (Ctx : in out Context; Val : RFLX.Flow_Gate.Window_Bytes)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Bytes)
       and then RFLX.Flow_Gate.Valid_Window_Bytes (RFLX.Flow_Gate.To_Base_Integer (Val))
       and then RFLX.Flow_Gate.Decision_Packet.Available_Space (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Bytes) >= RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Bytes)
       and then RFLX.Flow_Gate.Decision_Packet.Field_Condition (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Bytes),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Bytes)
       and Get_Bytes (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Bytes))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Bytes) = Valid_Next (Ctx, F_Bytes)'Old
       and Get_Kind (Ctx) = Get_Kind (Ctx)'Old
       and Field_First (Ctx, F_Bytes) = Field_First (Ctx, F_Bytes)'Old
   ...
   type Structure is
      record
         Kind : RFLX.Flow_Gate.Decision_Kind;
         Bytes : RFLX.Flow_Gate.Window_Bytes;
      end record;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Structure (Struct)
       and then RFLX.Flow_Gate.Decision_Packet.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Kind (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);

   function Field_Size_Bytes (Struct : Structure) return RFLX_Types.Bit_Length
   with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Kind)) then True)
      and then (if Well_Formed (Cursors (F_Bytes)) then Valid (Cursors (F_Kind))))
   with
     Pre =>
   ...
   function Field_First_Bytes (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First + 8)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Bytes);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_Bytes =>
             Field_First_Bytes (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                ...
                and then (if
                             Well_Formed (Cursors (F_Bytes))
                          then
                             (Cursors (F_Bytes).Last - Cursors (F_Bytes).First + 1 = 32
                              and then Cursors (F_Bytes).First = Cursors (F_Kind).Last + 1))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Flow_Gate.Decision_Packet.Field_First (Ctx, RFLX.Flow_Gate.Decision_Packet.F_Kind) rem RFLX_Types.Byte'Size = 1
   ...
   function Valid_Value (Fld : Field; Val : RFLX_Types.Base_Integer) return Boolean is
     (case Fld is
          when F_Kind =>
             RFLX.Flow_Gate.Valid_Decision_Kind (Val),
          when F_Bytes =>
             RFLX.Flow_Gate.Valid_Window_Bytes (Val));
   ...
   function Get_Kind (Ctx : Context) return RFLX.Flow_Gate.Decision_Kind is
     (To_Actual (Ctx.Cursors (F_Kind).Value));

   function Get_Bytes (Ctx : Context) return RFLX.Flow_Gate.Window_Bytes is
     (To_Actual (Ctx.Cursors (F_Bytes).Value));
   ...
   function Field_Size_Kind (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);

   function Field_Size_Bytes (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

end RFLX.Flow_Gate.Decision_Packet;
