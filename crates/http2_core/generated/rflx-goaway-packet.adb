...
package body RFLX.Goaway.Packet
with
  SPARK_Mode
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     (case Fld is
          when F_Reserved =>
             Invalid (Ctx.Cursors (F_Last_Stream_Id)),
          when F_Last_Stream_Id =>
             Invalid (Ctx.Cursors (F_Error_Code)),
          when F_Error_Code =>
             Invalid (Ctx.Cursors (F_Additional_Debug_Data)),
   ...
   function Composite_Field (Fld : Field) return Boolean is
     (Fld in F_Additional_Debug_Data);
   ...
   with
     Pre =>
       ...
       and then RFLX.Goaway.Packet.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Goaway.Packet.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Additional_Debug_Data then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   function Get_Additional_Debug_Data (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Additional_Debug_Data;

   procedure Get_Additional_Debug_Data (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).Last);
   begin
   ...
   procedure Generic_Get_Additional_Debug_Data (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Additional_Debug_Data).Last);
   begin
      Process_Additional_Debug_Data (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Additional_Debug_Data;
   ...
   with
     Pre =>
       RFLX.Goaway.Packet.Has_Buffer (Ctx)
       and then RFLX.Goaway.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Goaway.Packet.Valid_Value (Fld, Val)
       and then RFLX.Goaway.Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Goaway.Packet.Available_Space (Ctx, Fld)
       and then (if RFLX.Goaway.Packet.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Goaway.Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Reserved | F_Last_Stream_Id | F_Error_Code
       and then RFLX.Goaway.Packet.Valid_Value (Fld, Val)
       and then RFLX.Goaway.Packet.Valid_Size (Ctx, Fld, RFLX.Goaway.Packet.Field_Size (Ctx, Fld))
       and then RFLX.Goaway.Packet.Available_Space (Ctx, Fld) >= RFLX.Goaway.Packet.Field_Size (Ctx, Fld)
       and then RFLX.Goaway.Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Goaway.Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Reserved (Ctx : in out Context; Val : RFLX.Goaway.Reserved_Bit) is
   begin
      Set_Scalar (Ctx, F_Reserved, RFLX.Goaway.To_Base_Integer (Val));
   end Set_Reserved;

   procedure Set_Last_Stream_Id (Ctx : in out Context; Val : RFLX.Goaway.Last_Stream_Id) is
   begin
      Set_Scalar (Ctx, F_Last_Stream_Id, RFLX.Goaway.To_Base_Integer (Val));
   end Set_Last_Stream_Id;

   procedure Set_Error_Code (Ctx : in out Context; Val : RFLX.Goaway.Error_Code) is
   begin
      Set_Scalar (Ctx, F_Error_Code, RFLX.Goaway.To_Base_Integer (Val));
   end Set_Error_Code;

   procedure Set_Additional_Debug_Data_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Additional_Debug_Data, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Additional_Debug_Data_Empty;

   procedure Initialize_Additional_Debug_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Goaway.Packet.Has_Buffer (Ctx)
     ...
     Post =>
       ...
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Additional_Debug_Data)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Additional_Debug_Data);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Additional_Debug_Data) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Additional_Debug_Data);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Additional_Debug_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0);
   end Initialize_Additional_Debug_Data_Private;

   procedure Initialize_Additional_Debug_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
   begin
      Initialize_Additional_Debug_Data_Private (Ctx, Length);
   end Initialize_Additional_Debug_Data;

   procedure Set_Additional_Debug_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Additional_Debug_Data));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Additional_Debug_Data_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Additional_Debug_Data)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Additional_Debug_Data)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Additional_Debug_Data))) = Data);
   end Set_Additional_Debug_Data;

   procedure Generic_Set_Additional_Debug_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Additional_Debug_Data));
   begin
      if Length > 0 then
         Process_Additional_Debug_Data (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.Goaway.Packet.Valid_Length (Ctx, RFLX.Goaway.Packet.F_Additional_Debug_Data, Length));
      Initialize_Additional_Debug_Data_Private (Ctx, Length);
   end Generic_Set_Additional_Debug_Data;
