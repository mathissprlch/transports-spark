...
package body RFLX.Settings.Parameter
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Settings.Parameter.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Settings.Parameter.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   with
     Pre =>
       RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, Fld)
       and then RFLX.Settings.Parameter.Valid_Value (Fld, Val)
       and then RFLX.Settings.Parameter.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Settings.Parameter.Available_Space (Ctx, Fld)
   ...
   with
     Pre =>
       ...
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, Fld)
       and then Fld in F_Identifier | F_Value
       and then RFLX.Settings.Parameter.Valid_Value (Fld, Val)
       and then RFLX.Settings.Parameter.Valid_Size (Ctx, Fld, RFLX.Settings.Parameter.Field_Size (Ctx, Fld))
       and then RFLX.Settings.Parameter.Available_Space (Ctx, Fld) >= RFLX.Settings.Parameter.Field_Size (Ctx, Fld)
       and then RFLX.Settings.Parameter.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Settings.Parameter.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Identifier (Ctx : in out Context; Val : RFLX.Http2_Parameters.HTTP_2_Settings_Enum) is
   begin
      Set_Scalar (Ctx, F_Identifier, RFLX.Http2_Parameters.To_Base_Integer (Val));
   end Set_Identifier;

   procedure Set_Value (Ctx : in out Context; Val : RFLX.Settings.Setting_Value) is
   begin
      Set_Scalar (Ctx, F_Value, RFLX.Settings.To_Base_Integer (Val));
   end Set_Value;

   procedure Set_Identifier (Ctx : in out Context; Val : RFLX.Http2_Parameters.HTTP_2_Settings)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Parameter.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter.Valid_Next (Ctx, RFLX.Settings.Parameter.F_Identifier)
       and then RFLX.Http2_Parameters.Valid_HTTP_2_Settings (Val)
   ...
   begin
      Struct.Identifier := Get_Identifier (Ctx);
   ...
   begin
      Reset (Ctx);
      Set_Identifier (Ctx, Struct.Identifier);
