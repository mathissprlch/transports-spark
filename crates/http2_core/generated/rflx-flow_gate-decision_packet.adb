...
package body RFLX.Flow_Gate.Decision_Packet
with
  SPARK_Mode
is
   ...
   with
     Pre =>
       ...
       and RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, Fld);
   ...
   with
     Pre =>
       ...
       and then RFLX.Flow_Gate.Decision_Packet.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert (if Fld = F_Bytes then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0);
   ...
   with
     Pre =>
       RFLX.Flow_Gate.Decision_Packet.Has_Buffer (Ctx)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Value (Fld, Val)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Flow_Gate.Decision_Packet.Available_Space (Ctx, Fld)
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Bytes and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   ...
   with
     Pre =>
       ...
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Next (Ctx, Fld)
       and then Fld in F_Kind | F_Bytes
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Value (Fld, Val)
       and then RFLX.Flow_Gate.Decision_Packet.Valid_Size (Ctx, Fld, RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, Fld))
       and then RFLX.Flow_Gate.Decision_Packet.Available_Space (Ctx, Fld) >= RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, Fld)
       and then RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Flow_Gate.Decision_Packet.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Kind (Ctx : in out Context; Val : RFLX.Flow_Gate.Decision_Kind) is
   begin
      Set_Scalar (Ctx, F_Kind, RFLX.Flow_Gate.To_Base_Integer (Val));
   end Set_Kind;

   procedure Set_Bytes (Ctx : in out Context; Val : RFLX.Flow_Gate.Window_Bytes) is
   begin
      Set_Scalar (Ctx, F_Bytes, RFLX.Flow_Gate.To_Base_Integer (Val));
   end Set_Bytes;

   procedure To_Structure (Ctx : Context; Struct : out Structure) is
   begin
      Struct.Kind := Get_Kind (Ctx);
      Struct.Bytes := Get_Bytes (Ctx);
   end To_Structure;

   procedure To_Context (Struct : Structure; Ctx : in out Context) is
   begin
      Reset (Ctx);
      Set_Kind (Ctx, Struct.Kind);
      Set_Bytes (Ctx, Struct.Bytes);
   end To_Context;
