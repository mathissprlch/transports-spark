...
with RFLX.Settings.Parameter_List;

package RFLX.Settings.Packet
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   type Virtual_Field is (F_Initial, F_Parameters, F_Final);

   subtype Field is Virtual_Field range F_Parameters .. F_Parameters;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Settings.Packet.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Has_Buffer (Ctx),
     Post =>
       not Has_Buffer (Ctx)
       and then Buffer /= null
       and then Buffer'Length > 0
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Packet.Well_Formed_Message (Ctx)
       and then RFLX.Settings.Packet.Byte_Size (Ctx) = Buffer'Length;
   ...
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then Offset < RFLX.Settings.Packet.Buffer_Length (Ctx)
       and then Pre (RFLX.Settings.Packet.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Packet.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Settings.Packet.Byte_Size (Ctx);
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Packet.Valid_Next (Ctx, Fld)
       and then RFLX.Settings.Packet.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Valid_Next (Ctx, Fld),
     Post =>
       (case Fld is
           when F_Parameters =>
   ...
   with
     Pre =>
       RFLX.Settings.Packet.Has_Buffer (Ctx)
       and RFLX.Settings.Packet.Valid_Next (Ctx, Fld);
   ...
   procedure Set_Parameters_Empty (Ctx : in out Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Packet.Valid_Next (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Available_Space (Ctx, RFLX.Settings.Packet.F_Parameters) >= RFLX.Settings.Packet.Field_Size (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Field_Condition (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Field_Size (Ctx, RFLX.Settings.Packet.F_Parameters) = 0,
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Parameters)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Parameters))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Parameters) = Valid_Next (Ctx, F_Parameters)'Old
       and Field_First (Ctx, F_Parameters) = Field_First (Ctx, F_Parameters)'Old;

   procedure Set_Parameters (Ctx : in out Context; Seq_Ctx : RFLX.Settings.Parameter_List.Context)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Packet.Has_Buffer (Ctx)
       ...
       and then RFLX.Settings.Packet.Available_Space (Ctx, RFLX.Settings.Packet.F_Parameters) >= RFLX.Settings.Packet.Field_Size (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Field_Condition (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Valid_Length (Ctx, RFLX.Settings.Packet.F_Parameters, RFLX.Settings.Parameter_List.Byte_Size (Seq_Ctx))
       and then RFLX.Settings.Parameter_List.Has_Buffer (Seq_Ctx)
       and then RFLX.Settings.Parameter_List.Valid (Seq_Ctx),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Parameters)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Parameters))
       ...
       and Valid_Next (Ctx, F_Parameters) = Valid_Next (Ctx, F_Parameters)'Old
       and Field_First (Ctx, F_Parameters) = Field_First (Ctx, F_Parameters)'Old
       and (if Field_Size (Ctx, F_Parameters) > 0 then Present (Ctx, F_Parameters));

   procedure Initialize_Parameters (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Packet.Valid_Next (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Valid_Length (Ctx, RFLX.Settings.Packet.F_Parameters, Length)
       and then RFLX.Settings.Packet.Available_Space (Ctx, RFLX.Settings.Packet.F_Parameters) >= RFLX_Types.To_Bit_Length (Length),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Parameters)
       and then Field_Size (Ctx, F_Parameters) = RFLX_Types.To_Bit_Length (Length)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Parameters))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Valid_Next (Ctx, F_Parameters) = Valid_Next (Ctx, F_Parameters)'Old
       and then Field_First (Ctx, F_Parameters) = Field_First (Ctx, F_Parameters)'Old;

   procedure Switch_To_Parameters (Ctx : in out Context; Seq_Ctx : out RFLX.Settings.Parameter_List.Context)
   with
     Pre =>
       not Ctx'Constrained
       ...
       and then RFLX.Settings.Packet.Valid_Next (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Field_Size (Ctx, RFLX.Settings.Packet.F_Parameters) > 0
       and then RFLX.Settings.Packet.Field_First (Ctx, RFLX.Settings.Packet.F_Parameters) rem RFLX_Types.Byte'Size = 1
       and then RFLX.Settings.Packet.Available_Space (Ctx, RFLX.Settings.Packet.F_Parameters) >= RFLX.Settings.Packet.Field_Size (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then RFLX.Settings.Packet.Field_Condition (Ctx, RFLX.Settings.Packet.F_Parameters),
     Post =>
       not RFLX.Settings.Packet.Has_Buffer (Ctx)
       and RFLX.Settings.Parameter_List.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and Seq_Ctx.First = Field_First (Ctx, F_Parameters)
       and Seq_Ctx.Last = Field_Last (Ctx, F_Parameters)
       and RFLX.Settings.Parameter_List.Valid (Seq_Ctx)
       and RFLX.Settings.Parameter_List.Sequence_Last (Seq_Ctx) = Seq_Ctx.First - 1
       and Present (Ctx, F_Parameters)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Field_Last (Ctx, F_Parameters) = Field_Last (Ctx, F_Parameters)'Old,
     Contract_Cases =>
       (Well_Formed (Ctx, F_Parameters) =>
           True,
        others =>
           True);

   function Complete_Parameters (Ctx : Context; Seq_Ctx : RFLX.Settings.Parameter_List.Context) return Boolean
   with
     Pre =>
       RFLX.Settings.Packet.Valid_Next (Ctx, RFLX.Settings.Packet.F_Parameters);

   procedure Update_Parameters (Ctx : in out Context; Seq_Ctx : in out RFLX.Settings.Parameter_List.Context)
   with
     Pre =>
       RFLX.Settings.Packet.Present (Ctx, RFLX.Settings.Packet.F_Parameters)
       and then not RFLX.Settings.Packet.Has_Buffer (Ctx)
       and then RFLX.Settings.Parameter_List.Has_Buffer (Seq_Ctx)
       and then Ctx.Buffer_First = Seq_Ctx.Buffer_First
       and then Ctx.Buffer_Last = Seq_Ctx.Buffer_Last
       and then Seq_Ctx.First = Field_First (Ctx, F_Parameters)
       and then Seq_Ctx.Last = Field_Last (Ctx, F_Parameters),
     Post =>
       (if
           RFLX.Settings.Packet.Complete_Parameters (Ctx, Seq_Ctx)
        then
           Present (Ctx, F_Parameters)
        else
           Invalid (Ctx, F_Parameters))
       and Has_Buffer (Ctx)
       and not RFLX.Settings.Parameter_List.Has_Buffer (Seq_Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and RFLX.Settings.Parameter_List.Valid (Seq_Ctx) = RFLX.Settings.Parameter_List.Valid (Seq_Ctx)'Old
       and RFLX.Settings.Parameter_List.Size (Seq_Ctx) = RFLX.Settings.Parameter_List.Size (Seq_Ctx)'Old
       and Field_First (Ctx, F_Parameters) = Field_First (Ctx, F_Parameters)'Old
       and Field_Size (Ctx, F_Parameters) = Field_Size (Ctx, F_Parameters)'Old,
     Depends =>
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return Boolean is
     ((if Well_Formed (Cursors (F_Parameters)) then True))
   with
     Pre =>
   ...
   function Field_First_Parameters (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length) return RFLX_Types.Bit_Index'Base is
     (First)
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, F_Parameters);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Parameters =>
             Field_First_Parameters (Cursors, First, Verified_Last, Written_Last))
   with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last)
      and then ((if
                    Well_Formed (Cursors (F_Parameters))
                 then
                    (Cursors (F_Parameters).Last - Cursors (F_Parameters).First + 1 = RFLX_Types.Bit_Length (Written_Last - First + 1)
                     and then Cursors (F_Parameters).First = First))))
   with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Parameters)
      and then RFLX.Settings.Packet.Field_First (Ctx, RFLX.Settings.Packet.F_Parameters) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Parameters) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Parameters));
   ...
   function Complete_Parameters (Ctx : Context; Seq_Ctx : RFLX.Settings.Parameter_List.Context) return Boolean is
     (RFLX.Settings.Parameter_List.Valid (Seq_Ctx)
      and RFLX.Settings.Parameter_List.Size (Seq_Ctx) = Field_Size (Ctx, F_Parameters));
