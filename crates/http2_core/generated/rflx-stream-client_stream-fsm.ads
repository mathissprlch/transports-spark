...
package RFLX.Stream.Client_Stream.FSM
with
  SPARK_Mode
is
   ...
   type State is (S_Loading_Headers, S_Sending_Headers, S_Loading_Data, S_Sending_Data, S_Awaiting_Reply, S_Forwarding_Inbound, S_Forwarding_Connection_Frame, S_Forwarding_Reset, S_Final);
     ...
     Post =>
       ...
       and then (if
                 ...
                 then
                    ...
                    and Stream.Client_Stream.FSM.Written_Last (Ctx, B_Inbound) = Stream.Client_Stream.FSM.Written_Last (Ctx, B_Inbound)'Old)
       and then (if
                    Ext_Buf /= B_Outgoing
                 then
                    Has_Buffer (Ctx, B_Outgoing) = Has_Buffer (Ctx, B_Outgoing)'Old
                    and Stream.Client_Stream.FSM.Written_Last (Ctx, B_Outgoing) = Stream.Client_Stream.FSM.Written_Last (Ctx, B_Outgoing)'Old)
...
private
   ...
   function Channel_Accessible (St : State; Chan : Channel) return Boolean is
     (case Chan is
          when C_Network =>
             (case St is
                 when S_Sending_Headers | S_Sending_Data | S_Awaiting_Reply =>
                    True,
                 when others =>
                    False),
          when C_App_Pending =>
