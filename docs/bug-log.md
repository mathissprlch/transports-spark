...
| 2026-05-22 | gnatprove functional-proof (ghost-bignum Mac Post) | `Tls_Core.Poly1305.Mac` finish | RFC 8439 §2.5.1 | (a)+(d) | Missing final reduction ("freeze"). The carry-fold (`Carry`) only guarantees Acc < 2^130, not Acc < p (= 2^130 - 5); the five values in [2^130-5, 2^130) are a fixed point of `Carry` (limb 4 < 2^26, so the x5 fold never fires) and reach the finish step un-reduced. `(Acc + s) mod 2^128` then diverges from the spec's `(Acc mod p + s) mod 2^128` by `p mod 2^128 = 2^128 - 5`. Window ~5/2^130 — invisible to RFC 8439 §2.5.2 test vectors and random/fuzz testing; only the functional Mac Post (`Out_Tag = Spec_Poly1305_Mac`, ported over `Tls_Core.Ghost_Bignum`) exposes it. Fix: constant-time conditional subtract of p before adding s, matching HACL* `poly1305_finish`. (d): shipped `[VERIFIED — AoRTE]`; the functional Post (in progress) catches it. Functional proof confirming the fix is pending the Big_Nat spec swap. | 6b0af3e |
