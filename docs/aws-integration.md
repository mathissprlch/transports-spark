...
| `grpc_ada` depends on patched AWS | **blocked** (see below) |
| `GRPC.Transport.HTTP2` body | **blocked** |
| End-to-end helloworld over the wire | **blocked** |

## What's blocking

The upstream Alire `aws=25.2.0` crate's `Post_Fetch` runs `make setup`, which
introspects system constants to generate `aws-os_lib.ads`. On macOS Apple
Silicon the resulting file is truncated to ~32 lines (just the header and a
single `pragma Style_Checks`) — every package body that does `with
AWS.OS_Lib;` then fails to compile with
...
2. **Vendor a known-good `aws-os_lib.ads` per platform** under
   `vendor/aws-overlays/darwin-arm64/aws-os_lib.ads`, copied in by
   `bootstrap.sh` after `make setup`. Cost: ~1 hour to capture the right
   contents from a working Linux build.
